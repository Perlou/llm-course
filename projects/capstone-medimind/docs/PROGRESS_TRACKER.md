...
| 1.3  | 创建 requirements.txt  | 0.5h     | ⬜ 待开始 |          |                      |
| 1.4  | 搭建 FastAPI 基础框架  | 2h       | ⬜ 待开始 |          | 路由、中间件         |
| 1.5  | 配置 PostgreSQL 数据库 | 1.5h     | ⬜ 待开始 |          | SQLAlchemy + asyncpg |
| 1.6  | 定义数据库模型         | 1.5h     | ⬜ 待开始 |          | 医学文档、药品、对话 |
| 1.7  | 配置 Chroma 向量存储   | 1h       | ⬜ 待开始 |          |                      |
| 1.8  | 集成 Gemini API        | 1h       | ⬜ 待开始 |          | 文本 + 视觉          |
| 1.9  | 集成 Ollama (备选)     | 1h       | ⬜ 待开始 |          | Qwen2.5-7B           |
| 1.10 | 配置日志系统           | 0.5h     | ⬜ 待开始 |          | loguru               |

**本周交付物**:

- [ ] 完整项目目录结构
- [ ] requirements.txt
- [ ] FastAPI 应用骨架 (src/api/main.py)
- [ ] 数据库模型定义 (src/models/)
- [ ] 基础配置文件 (configs/)
...
| #    | 任务                   | 预计耗时 | 状态      | 完成日期 | 备注                 |
| ---- | ---------------------- | -------- | --------- | -------- | -------------------- |
| 2.1  | 收集疾病科普数据       | 3h       | ⬜ 待开始 |          | 卫健委、权威医学网站 |
| 2.2  | 收集药品说明书数据     | 3h       | ⬜ 待开始 |          | 国家药监局数据库     |
| 2.3  | 收集检验指标参考数据   | 2h       | ⬜ 待开始 |          | 常见指标及正常范围   |
| 2.4  | 实现医学文档解析器     | 2h       | ⬜ 待开始 |          | PDF、Markdown        |
| 2.5  | 实现文本分块策略       | 2h       | ⬜ 待开始 |          | 医学文档特化分块     |
| 2.6  | 实现 Embedder 嵌入服务 | 1.5h     | ⬜ 待开始 |          | bge-large-zh         |
| 2.7  | 构建疾病知识向量库     | 2h       | ⬜ 待开始 |          |                      |
| 2.8  | 构建药品信息数据库     | 2h       | ⬜ 待开始 |          | 结构化存储           |
| 2.9  | 构建检验指标数据库     | 1h       | ⬜ 待开始 |          |                      |
| 2.10 | 编写数据加载脚本       | 1h       | ⬜ 待开始 |          | scripts/load_data.py |

**本周交付物**:

- [ ] 医学知识数据集 (data/medical_docs/)
- [ ] 药品数据库 (data/drug_db/)
- [ ] 检验指标库 (data/lab_indices/)
- [ ] 向量索引 (data/chroma_index/)
- [ ] 数据加载脚本
