"""
多 Agent 系统概述
=================

学习目标：
    1. 理解什么是多 Agent 系统
    2. 掌握多 Agent vs 单 Agent 的区别
    3. 了解多 Agent 系统的核心组件
    4. 理解多 Agent 的应用场景

核心概念：
    - 多 Agent 系统 (Multi-Agent System, MAS)
    - Agent 间协作与通信
    - 分布式问题解决

前置知识：
    - Phase 6: Agent 与工具基础
    - 异步编程基础

环境要求：
    - pip install openai python-dotenv
"""

import os
from dotenv import load_dotenv

load_dotenv()


# ==================== 第一部分：什么是多 Agent 系统 ====================


def what_is_multi_agent():
    """多 Agent 系统概述"""
    print("=" * 60)
    print("第一部分：什么是多 Agent 系统")
    print("=" * 60)

    print("""
    多 Agent 系统 (Multi-Agent System, MAS)
    ───────────────────────────────────────
    
    多个 Agent 协同工作，共同完成复杂任务的系统架构。
    
    ┌─────────────────────────────────────────────────────┐
    │               Multi-Agent System                     │
    │                                                     │
    │   ┌─────────┐    ┌─────────┐    ┌─────────┐       │
    │   │ Agent A │◄──►│ Agent B │◄──►│ Agent C │       │
    │   │ (规划)  │    │ (执行)  │    │ (验证)  │       │
    │   └────┬────┘    └────┬────┘    └────┬────┘       │
    │        │              │              │             │
    │        └──────────────┼──────────────┘             │
    │                       ▼                            │
    │          ┌─────────────────────────┐               │
    │          │     共享环境/知识库      │               │
    │          └─────────────────────────┘               │
    │                                                     │
    │   核心特点：自主性 | 协作性 | 分布式 | 专业化       │
    └─────────────────────────────────────────────────────┘
    
    现实世界类比：
    ─────────────
    🏢 公司团队：产品经理 + 开发工程师 + 测试工程师 + 运维
    🏥 医疗团队：主治医生 + 专科医生 + 护士 + 药剂师
    📰 新闻团队：记者 + 编辑 + 校对 + 摄影师
    """)


# ==================== 第二部分：单 Agent vs 多 Agent ====================


def single_vs_multi_agent():
    """单 Agent 与多 Agent 对比"""
    print("\n" + "=" * 60)
    print("第二部分：单 Agent vs 多 Agent")
    print("=" * 60)

    print("""
    ┌────────────────┬─────────────────────┬─────────────────────────┐
    │     维度       │      单 Agent       │        多 Agent         │
    ├────────────────┼─────────────────────┼─────────────────────────┤
    │  任务复杂度    │  简单到中等          │   复杂任务              │
    │  专业性        │  通用型，什么都做     │   各司其职，专业分工    │
    │  可扩展性      │  有限，难以扩展       │   高度可扩展            │
    │  容错能力      │  单点故障            │   分布式容错            │
    │  开发复杂度    │  低                  │   中到高                │
    │  通信开销      │  无                  │   需要协调通信          │
    │  适用场景      │  问答、简单任务       │   软件开发、研究、创作  │
    └────────────────┴─────────────────────┴─────────────────────────┘
    
    什么时候使用多 Agent？
    ─────────────────────
    
    ✅ 任务可以自然分解为多个子任务
    ✅ 不同子任务需要不同的专业知识
    ✅ 需要多角度审查或验证
    ✅ 任务涉及创意和批评的循环
    ✅ 需要模拟团队协作场景
    
    ❌ 简单的问答任务
    ❌ 单一明确的操作
    ❌ 对响应速度要求极高的场景
    """)


# ==================== 第三部分：多 Agent 核心组件 ====================


def multi_agent_components():
    """多 Agent 系统核心组件"""
    print("\n" + "=" * 60)
    print("第三部分：多 Agent 核心组件")
    print("=" * 60)

    print("""
    1. Agent（智能体）
    ─────────────────
    系统中的基本单元，具有自主决策能力
    
    2. 通信协议
    ─────────────────
    Agent 之间如何交换信息
    - 消息传递
    - 共享黑板
    - 订阅发布
    
    3. 协调机制
    ─────────────────
    如何组织 Agent 协同工作
    - 层级式：主管-工人模式
    - 扁平式：对等协作
    - 流水线式：顺序处理
    
    4. 共享状态
    ─────────────────
    Agent 共享的信息和知识
    - 共享内存
    - 黑板系统
    - 分布式状态
    """)

    # 代码示例：Agent 基础结构
    print("\n📌 代码示例：Agent 基础结构")

    from abc import ABC, abstractmethod
    from dataclasses import dataclass, field
    from typing import Dict, List, Any, Optional
    from enum import Enum
    import uuid

    class AgentRole(Enum):
        """Agent 角色类型"""

        COORDINATOR = "coordinator"  # 协调者
        EXECUTOR = "executor"  # 执行者
        VALIDATOR = "validator"  # 验证者
        RESEARCHER = "researcher"  # 研究者
        CRITIC = "critic"  # 评审者

    @dataclass
    class Message:
        """Agent 间通信消息"""

        id: str = field(default_factory=lambda: str(uuid.uuid4()))
        sender: str = ""
        receiver: str = ""  # 空字符串表示广播
        content: Any = None
        msg_type: str = "text"
        metadata: Dict = field(default_factory=dict)

    @dataclass
    class AgentState:
        """Agent 状态"""

        status: str = "idle"  # idle, working, waiting, completed
        current_task: Optional[str] = None
        memory: List[Dict] = field(default_factory=list)
        skills: List[str] = field(default_factory=list)

    class BaseAgent(ABC):
        """Agent 基类"""

        def __init__(self, name: str, role: AgentRole, description: str = ""):
            self.id = str(uuid.uuid4())
            self.name = name
            self.role = role
            self.description = description
            self.state = AgentState()
            self.inbox: List[Message] = []
            self.outbox: List[Message] = []

        @abstractmethod
        def process(self, input_data: Any) -> Any:
            """处理输入数据"""
            pass

        @abstractmethod
        def receive_message(self, message: Message):
            """接收消息"""
            pass

        def send_message(self, receiver: str, content: Any, msg_type: str = "text"):
            """发送消息"""
            message = Message(
                sender=self.name, receiver=receiver, content=content, msg_type=msg_type
            )
            self.outbox.append(message)
            return message

    # 打印类定义
    print("""
    class AgentRole(Enum):
        COORDINATOR = "coordinator"    # 协调者
        EXECUTOR = "executor"          # 执行者
        VALIDATOR = "validator"        # 验证者
        RESEARCHER = "researcher"      # 研究者
        CRITIC = "critic"              # 评审者

    @dataclass
    class Message:
        id: str
        sender: str
        receiver: str
        content: Any
        msg_type: str

    class BaseAgent(ABC):
        def __init__(self, name, role, description):
            self.id = uuid.uuid4()
            self.name = name
            self.role = role
            self.state = AgentState()
            self.inbox = []
            self.outbox = []
    """)


# ==================== 第四部分：多 Agent 架构模式 ====================


def architecture_patterns():
    """多 Agent 架构模式"""
    print("\n" + "=" * 60)
    print("第四部分：多 Agent 架构模式")
    print("=" * 60)

    print("""
    1. 层级式架构 (Hierarchical)
    ───────────────────────────
    
           ┌─────────────┐
           │  Supervisor │  主管 Agent
           └──────┬──────┘
          ┌───────┼───────┐
          ▼       ▼       ▼
       ┌─────┐ ┌─────┐ ┌─────┐
       │  A  │ │  B  │ │  C  │  工人 Agent
       └─────┘ └─────┘ └─────┘
    
    特点：清晰的命令链，易于管理
    适用：任务分解明确的场景
    
    2. 扁平式架构 (Flat/Peer)
    ─────────────────────────
    
       ┌─────┐     ┌─────┐
       │  A  │◄───►│  B  │
       └──┬──┘     └──┬──┘
          │    ▲     │
          ▼    │     ▼
       ┌─────┐─┴──┌─────┐
       │  D  │◄──►│  C  │
       └─────┘    └─────┘
    
    特点：所有 Agent 平等协作
    适用：需要多角度讨论的场景
    
    3. 流水线式架构 (Pipeline)
    ──────────────────────────
    
       ┌─────┐   ┌─────┐   ┌─────┐
       │  A  │──►│  B  │──►│  C  │
       └─────┘   └─────┘   └─────┘
         │         │         │
         ▼         ▼         ▼
      Stage1    Stage2    Stage3
    
    特点：顺序处理，每个 Agent 处理特定阶段
    适用：工作流明确的场景
    
    4. 混合式架构 (Hybrid)
    ──────────────────────
    
         ┌─────────────┐
         │ Orchestrator│
         └──────┬──────┘
          ┌─────┴─────┐
          ▼           ▼
       ┌─────┐     ┌─────┐
       │Team │     │Team │
       │  A  │     │  B  │
       └─────┘     └─────┘
    
    特点：结合多种模式的优点
    适用：复杂的企业级应用
    """)


# ==================== 第五部分：简单多 Agent 演示 ====================


def simple_multi_agent_demo():
    """简单多 Agent 系统演示"""
    print("\n" + "=" * 60)
    print("第五部分：简单多 Agent 系统演示")
    print("=" * 60)

    print("\n📌 演示：写作-审核两 Agent 协作\n")

    class WriterAgent:
        """写作 Agent"""

        def __init__(self):
            self.name = "Writer"

        def write(self, topic: str) -> str:
            """生成文章草稿"""
            # 模拟写作过程
            draft = f"""
            【关于{topic}的文章】
            
            第一段：介绍{topic}的基本概念...
            
            第二段：{topic}的重要性和应用...
            
            第三段：{topic}的未来发展趋势...
            """.strip()
            return draft

        def revise(self, draft: str, feedback: str) -> str:
            """根据反馈修改文章"""
            # 模拟修改过程
            revised = f"{draft}\n\n【根据反馈修改】\n- 已改进：{feedback}"
            return revised

    class ReviewerAgent:
        """审核 Agent"""

        def __init__(self):
            self.name = "Reviewer"

        def review(self, content: str) -> dict:
            """审核文章"""
            # 模拟审核过程
            issues = []
            suggestions = []

            if len(content) < 200:
                issues.append("文章内容过短")
                suggestions.append("建议增加更多细节和例子")

            if "例如" not in content and "例子" not in content:
                issues.append("缺少具体例子")
                suggestions.append("请添加实际案例")

            return {
                "approved": len(issues) == 0,
                "issues": issues,
                "suggestions": suggestions,
                "score": 7 if issues else 9,
            }

    class SimpleMultiAgentSystem:
        """简单多 Agent 系统"""

        def __init__(self):
            self.writer = WriterAgent()
            self.reviewer = ReviewerAgent()

        def run(self, topic: str, max_iterations: int = 3) -> dict:
            """运行多 Agent 协作流程"""
            print(f"📋 任务: 撰写关于「{topic}」的文章\n")
            print("-" * 40)

            # 写作阶段
            print(f"✍️  {self.writer.name} 正在写作...")
            draft = self.writer.write(topic)
            print(f"📝 草稿完成:\n{draft[:100]}...\n")

            # 审核循环
            for i in range(max_iterations):
                print(f"🔍 {self.reviewer.name} 正在审核 (第 {i + 1} 轮)...")
                review = self.reviewer.review(draft)

                print(f"   评分: {review['score']}/10")
                print(f"   通过: {'✅ 是' if review['approved'] else '❌ 否'}")

                if review["approved"]:
                    print("✅ 审核通过！")
                    break

                print(f"   问题: {review['issues']}")
                print(f"   建议: {review['suggestions']}")

                # 修改
                print(f"\n✍️  {self.writer.name} 正在修改...")
                feedback = "; ".join(review["suggestions"])
                draft = self.writer.revise(draft, feedback)
                print()

            return {
                "topic": topic,
                "final_content": draft,
                "iterations": i + 1,
                "final_review": review,
            }

    # 运行演示
    print("-" * 40)
    system = SimpleMultiAgentSystem()
    result = system.run("人工智能的未来")

    print("\n" + "=" * 40)
    print("📊 协作结果:")
    print(f"   总迭代次数: {result['iterations']}")
    print(f"   最终评分: {result['final_review']['score']}/10")


# ==================== 第六部分：应用场景 ====================


def application_scenarios():
    """多 Agent 应用场景"""
    print("\n" + "=" * 60)
    print("第六部分：多 Agent 应用场景")
    print("=" * 60)

    print("""
    典型应用场景
    ───────────
    
    1. 软件开发团队
       ┌─────────┐   ┌─────────┐   ┌─────────┐   ┌─────────┐
       │   PM    │──►│Developer│──►│Reviewer │──►│ Tester  │
       └─────────┘   └─────────┘   └─────────┘   └─────────┘
       需求分析      代码编写       代码审查       测试验证
    
    2. 内容创作团队
       ┌─────────┐   ┌─────────┐   ┌─────────┐
       │ Writer  │──►│ Editor  │──►│ Proofer │
       └─────────┘   └─────────┘   └─────────┘
       初稿撰写      内容编辑       校对审核
    
    3. 研究分析团队
       ┌─────────┐   ┌─────────┐   ┌─────────┐
       │Searcher │──►│Analyzer │──►│Reporter │
       └─────────┘   └─────────┘   └─────────┘
       信息收集      数据分析       报告生成
    
    4. 辩论/头脑风暴
       ┌─────────┐◄──►┌─────────┐
       │Supporter│    │ Critic  │
       └────┬────┘    └────┬────┘
            │              │
            └──────┬───────┘
                   ▼
            ┌─────────┐
            │Moderator│  总结双方观点
            └─────────┘
    
    5. 客户服务
       ┌─────────┐   ┌─────────┐   ┌─────────┐
       │Classifier│──►│ Handler │──►│ QA Check│
       └─────────┘   └─────────┘   └─────────┘
       问题分类      问题处理       质量检查
    """)


# ==================== 第七部分：练习与思考 ====================


def exercises():
    """练习题"""
    print("\n" + "=" * 60)
    print("练习与思考")
    print("=" * 60)

    print("""
    练习 1：扩展写作系统
        为上面的多 Agent 系统添加一个 Editor Agent，
        负责在 Writer 和 Reviewer 之间进行内容优化。
    
    练习 2：设计一个辩论系统
        创建两个 Agent（正方和反方）就某个话题进行辩论，
        由第三个 Agent 作为裁判总结双方观点。
    
    练习 3：实现消息传递
        为 BaseAgent 实现一个简单的消息队列，
        让 Agent 之间可以异步通信。
    
    思考题：
    ────────
    1. 多 Agent 系统中如何避免死锁？
       答：设置超时机制、定义明确的通信协议、
       使用异步通信、避免循环依赖。
    
    2. 如何确保 Agent 之间的信息一致性？
       答：使用共享状态管理、设置版本控制、
       实现事务机制、定期同步状态。
    
    3. 何时应该增加 Agent 数量？何时应该减少？
       答：任务可以明确分解时增加，Agent 间通信开销
       大于收益时减少。需要权衡复杂度和效率。
    """)


# ==================== 主函数 ====================


def main():
    """主函数"""
    print("🤖 多 Agent 系统概述")
    print("=" * 60)

    what_is_multi_agent()
    single_vs_multi_agent()
    multi_agent_components()
    architecture_patterns()
    simple_multi_agent_demo()
    application_scenarios()
    exercises()

    print("\n" + "=" * 60)
    print("✅ 课程完成！下一步：02-langgraph-basics.py")
    print("=" * 60)


if __name__ == "__main__":
    main()
