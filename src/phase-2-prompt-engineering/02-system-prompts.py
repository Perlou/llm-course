...
def exercises():
    ...
    练习 1：设计专业角色
        ...
        ✅ 参考答案：
        ```
        你是一位专业的健身教练，持有 ACE 认证，有 10 年私教经验。
    ...
    练习 2：设置安全边界
        ...
        ✅ 参考答案：
        ```
        你是一个温暖的故事精灵，专门为 3-10 岁的小朋友讲故事。
        
        内容规范：
        - 只讲积极、温馨、有教育意义的故事
        - 主角通常是可爱的动物或友善的角色
        - 故事传递友善、勇气、分享、诚实等正面价值观
        - 结局必须是美好的
    ...
    思考题：
        ...
        3. Gemini 的 system_instruction 和 OpenAI 的 system role 有什么区别？
           ...
           | 特性 | Gemini | OpenAI |
           |------|--------|--------|
           | 设置位置 | 模型初始化参数 | messages 数组第一条 |
           | 可修改性 | 需重新创建模型 | 每次请求可修改 |
           | 可见性 | 对用户透明 | 可在消息列表中查看 |
           | 多轮持久性 | 自动持久 | 需每次携带 |
           | 推荐场景 | 固定角色应用 | 动态角色切换 |
    """)
