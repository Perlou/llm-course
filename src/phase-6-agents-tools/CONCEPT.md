# Phase 6: Agent 与工具调用 - 核心概念深度解析

> **学习目标**：深入理解 Agent 架构、工具调用机制以及 MCP 协议

---

## 目录

1. [引言：什么是 Agent？](#1-引言什么是-agent)
2. [Agent 核心架构](#2-agent-核心架构)
3. [工具调用机制 (Tool Calling)](#3-工具调用机制-tool-calling)
4. [ReAct 模式：推理与行动](#4-react-模式推理与行动)
5. [MCP 协议详解](#5-mcp-协议详解)
6. [实战：构建完整 Agent](#6-实战构建完整-agent)
7. [最佳实践与常见陷阱](#7-最佳实践与常见陷阱)
8. [总结](#8-总结)

---

## 1. 引言：什么是 Agent？

### 1.1 Agent 的定义

```
┌─────────────────────────────────────────────────────────────────┐
│                        什么是 Agent？                            │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   Agent = LLM + 记忆 + 工具 + 规划能力                           │
│                                                                 │
│   ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐     │
│   │   LLM   │ +  │  Memory │ +  │  Tools  │ +  │Planning │     │
│   │  大脑   │    │  记忆   │    │  工具   │    │  规划   │     │
│   └─────────┘    └─────────┘    └─────────┘    └─────────┘     │
│                                                                 │
│   Agent 是一个能够自主感知环境、做出决策、执行行动的智能系统       │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

**Agent（智能体）** 是一个以大语言模型为核心的自主系统，它能够：

- 🧠 **理解**用户意图和上下文
- 🤔 **规划**完成任务的步骤
- 🔧 **使用**外部工具扩展能力
- 🔄 **迭代**优化直到任务完成

### 1.2 传统 LLM vs Agent

```
┌────────────────────────────────────────────────────────────────────┐
│                    传统 LLM vs Agent 对比                          │
├──────────────────────┬─────────────────────────────────────────────┤
│      传统 LLM        │                 Agent                       │
├──────────────────────┼─────────────────────────────────────────────┤
│                      │                                             │
│  用户 ──→ LLM ──→ 回复  │  用户 ──→ Agent ──→ 思考 ──→ 行动 ──→ 观察  │
│                      │              ↑                    │         │
│   • 单轮对话          │              └────────────────────┘         │
│   • 静态知识          │                                             │
│   • 无法执行操作       │   • 多轮推理循环                            │
│   • 被动响应          │   • 动态获取信息                             │
│                      │   • 可执行真实操作                           │
│                      │   • 主动完成任务                             │
│                      │                                             │
└──────────────────────┴─────────────────────────────────────────────┘
```

### 1.3 为什么需要 Agent？

```python
# 传统 LLM 的局限性示例
user_question = "今天北京的天气怎么样？"

# ❌ 传统 LLM 只能基于训练数据回答
traditional_response = "我无法获取实时天气信息，我的知识截止于..."

# ✅ Agent 可以调用工具获取实时信息
agent_response = """
让我查询一下北京的实时天气...
[调用天气API]
根据查询结果，今天北京天气晴朗，气温25°C，适合外出。
"""
```

**Agent 解决了 LLM 的核心局限**：

| 局限性       | Agent 解决方案           |
| ------------ | ------------------------ |
| 知识过时     | 通过搜索工具获取最新信息 |
| 无法计算     | 调用计算器/代码执行器    |
| 无法操作     | 集成 API 执行真实操作    |
| 缺乏专业知识 | 接入知识库和专业工具     |

---

## 2. Agent 核心架构

### 2.1 基本架构图

```
┌─────────────────────────────────────────────────────────────────────┐
│                         Agent 核心架构                              │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│                        ┌───────────────┐                            │
│                        │   用户输入     │                            │
│                        └───────┬───────┘                            │
│                                │                                    │
│                                ▼                                    │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │                      Agent 核心层                             │  │
│  │  ┌─────────────────────────────────────────────────────────┐ │  │
│  │  │                    LLM (大脑)                            │ │  │
│  │  │  • 理解输入    • 推理决策    • 生成输出                   │ │  │
│  │  └─────────────────────────────────────────────────────────┘ │  │
│  │                             │                                 │  │
│  │  ┌──────────┬──────────┬────┴─────┬───────────┐              │  │
│  │  ▼          ▼          ▼          ▼           │              │  │
│  │ ┌────┐   ┌─────┐   ┌──────┐   ┌──────┐       │              │  │
│  │ │记忆│   │规划 │   │ 工具 │   │ 反思 │       │              │  │
│  │ └────┘   └─────┘   └──────┘   └──────┘       │              │  │
│  │   │         │          │          │          │              │  │
│  └───┼─────────┼──────────┼──────────┼──────────┼──────────────┘  │
│      │         │          │          │          │                  │
│      ▼         ▼          ▼          ▼          ▼                  │
│  ┌──────┐ ┌───────┐ ┌──────────┐ ┌───────┐ ┌────────┐             │
│  │短期  │ │任务   │ │ 外部API  │ │结果   │ │ 输出   │             │
│  │长期  │ │分解   │ │ 数据库   │ │评估   │ │ 响应   │             │
│  │记忆  │ │       │ │ 文件系统 │ │       │ │        │             │
│  └──────┘ └───────┘ └──────────┘ └───────┘ └────────┘             │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 2.2 核心组件详解

#### 2.2.1 LLM（大脑）

```python
class AgentBrain:
    """Agent 的大脑 - LLM"""

    def __init__(self, model: str = "gpt-4"):
        self.model = model
        self.system_prompt = """
        你是一个智能助手，可以使用工具来完成任务。
        在回答问题时，请遵循以下步骤：
        1. 分析用户需求
        2. 决定是否需要使用工具
        3. 如需使用工具，调用适当的工具
        4. 基于工具返回结果，给出最终答案
        """

    def think(self, input: str, context: dict) -> dict:
        """思考并决策下一步行动"""
        # 调用 LLM 进行推理
        response = self.call_llm(input, context)
        return self.parse_response(response)
```

#### 2.2.2 记忆系统

```python
class MemorySystem:
    """Agent 的记忆系统"""

    def __init__(self):
        # 短期记忆：当前对话上下文
        self.short_term: List[Message] = []

        # 长期记忆：持久化存储
        self.long_term: VectorStore = VectorStore()

        # 工作记忆：当前任务状态
        self.working: Dict[str, Any] = {}

    def add_to_short_term(self, message: Message):
        """添加到短期记忆"""
        self.short_term.append(message)
        # 保持窗口大小
        if len(self.short_term) > self.max_size:
            self.compress_and_store()

    def recall(self, query: str) -> List[str]:
        """从长期记忆中检索相关信息"""
        return self.long_term.search(query, top_k=5)
```

```
┌─────────────────────────────────────────────────────────────────┐
│                        记忆系统架构                              │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   ┌─────────────────────────────────────────────────────────┐  │
│   │                    短期记忆                              │  │
│   │   ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐              │  │
│   │   │ M1  │→│ M2  │→│ M3  │→│ M4  │→│ M5  │  (对话历史)   │  │
│   │   └─────┘ └─────┘ └─────┘ └─────┘ └─────┘              │  │
│   └──────────────────────────┬──────────────────────────────┘  │
│                              │ 压缩/总结                        │
│                              ▼                                  │
│   ┌─────────────────────────────────────────────────────────┐  │
│   │                    长期记忆                              │  │
│   │   ┌─────────────────────────────────────────────────┐   │  │
│   │   │              向量数据库                          │   │  │
│   │   │   [知识片段] [历史总结] [学习经验] [用户偏好]    │   │  │
│   │   └─────────────────────────────────────────────────┘   │  │
│   └─────────────────────────────────────────────────────────┘  │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

#### 2.2.3 规划模块

```python
class PlanningModule:
    """任务规划模块"""

    def decompose_task(self, complex_task: str) -> List[SubTask]:
        """将复杂任务分解为子任务"""
        prompt = f"""
        请将以下任务分解为可执行的子任务：
        任务：{complex_task}

        输出格式：
        1. [子任务1]
        2. [子任务2]
        ...
        """
        # 调用 LLM 进行任务分解
        return self.llm.generate(prompt)

    def create_plan(self, goal: str, available_tools: List[Tool]) -> Plan:
        """创建执行计划"""
        return Plan(
            goal=goal,
            steps=self.decompose_task(goal),
            tools=self.select_tools(available_tools)
        )
```

### 2.3 Agent 执行循环

```
┌─────────────────────────────────────────────────────────────────────┐
│                    Agent 执行循环 (Agent Loop)                       │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│           ┌──────────────────────────────────────────┐              │
│           │                                          │              │
│           ▼                                          │              │
│    ┌─────────────┐                                   │              │
│    │   感知      │  ← 接收用户输入/环境反馈           │              │
│    │  Perceive   │                                   │              │
│    └──────┬──────┘                                   │              │
│           │                                          │              │
│           ▼                                          │              │
│    ┌─────────────┐                                   │              │
│    │   思考      │  ← LLM 推理、规划                  │              │
│    │   Think     │                                   │              │
│    └──────┬──────┘                                   │              │
│           │                                          │              │
│           ▼                                          │              │
│    ┌─────────────┐      ┌─────────────┐              │              │
│    │   决策      │──是──│  需要工具？  │              │              │
│    │  Decide     │      └──────┬──────┘              │              │
│    └──────┬──────┘             │否                   │              │
│           │                    ▼                     │              │
│           │             ┌─────────────┐              │              │
│           │             │  直接回复   │──────────────┤              │
│           ▼             └─────────────┘              │              │
│    ┌─────────────┐                                   │              │
│    │   执行      │  ← 调用工具/执行操作               │              │
│    │    Act      │                                   │              │
│    └──────┬──────┘                                   │              │
│           │                                          │              │
│           ▼                                          │              │
│    ┌─────────────┐      ┌─────────────┐              │              │
│    │   观察      │──否──│  任务完成？  │──────────────┘              │
│    │  Observe    │      └──────┬──────┘                             │
│    └─────────────┘             │是                                  │
│                                ▼                                    │
│                         ┌─────────────┐                             │
│                         │  返回结果   │                             │
│                         └─────────────┘                             │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

```python
class AgentLoop:
    """Agent 核心执行循环"""

    def run(self, user_input: str) -> str:
        """执行 Agent 循环"""

        # 初始化
        self.memory.add_message("user", user_input)
        iteration = 0
        max_iterations = 10

        while iteration < max_iterations:
            iteration += 1

            # 1. 思考：LLM 决定下一步行动
            thought = self.think()

            # 2. 检查是否需要工具
            if thought.action_type == "final_answer":
                return thought.content

            # 3. 执行工具调用
            if thought.action_type == "tool_call":
                tool_name = thought.tool_name
                tool_args = thought.tool_args

                # 执行工具
                observation = self.execute_tool(tool_name, tool_args)

                # 4. 将观察结果加入上下文
                self.memory.add_observation(observation)

        return "达到最大迭代次数，任务未完成"
```

---

## 3. 工具调用机制 (Tool Calling)

### 3.1 什么是工具调用？

```
┌─────────────────────────────────────────────────────────────────────┐
│                         工具调用流程                                 │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  用户: "帮我查询北京到上海的机票"                                     │
│          │                                                          │
│          ▼                                                          │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                         LLM 分析                               │ │
│  │  "用户需要查询机票，我需要调用机票查询工具"                       │ │
│  └───────────────────────────────────────────────────────────────┘ │
│          │                                                          │
│          ▼                                                          │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                      生成工具调用请求                           │ │
│  │  {                                                             │ │
│  │    "tool": "search_flights",                                   │ │
│  │    "arguments": {                                              │ │
│  │      "departure": "北京",                                      │ │
│  │      "arrival": "上海",                                        │ │
│  │      "date": "2024-01-15"                                      │ │
│  │    }                                                           │ │
│  │  }                                                             │ │
│  └───────────────────────────────────────────────────────────────┘ │
│          │                                                          │
│          ▼                                                          │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                      执行工具 & 返回结果                        │ │
│  │  航班列表: [CA1234, MU5678, ...]                               │ │
│  └───────────────────────────────────────────────────────────────┘ │
│          │                                                          │
│          ▼                                                          │
│  ┌───────────────────────────────────────────────────────────────┐ │
│  │                      LLM 整合回复                               │ │
│  │  "为您找到以下航班: CA1234 08:00起飞..."                        │ │
│  └───────────────────────────────────────────────────────────────┘ │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 3.2 工具定义规范

```python
from typing import Callable, Any
from pydantic import BaseModel, Field

# 方式一：使用 JSON Schema 定义工具
tool_definition = {
    "type": "function",
    "function": {
        "name": "get_weather",
        "description": "获取指定城市的当前天气信息",
        "parameters": {
            "type": "object",
            "properties": {
                "city": {
                    "type": "string",
                    "description": "城市名称，如 '北京'"
                },
                "unit": {
                    "type": "string",
                    "enum": ["celsius", "fahrenheit"],
                    "description": "温度单位"
                }
            },
            "required": ["city"]
        }
    }
}

# 方式二：使用 Pydantic 模型定义（推荐）
class GetWeatherInput(BaseModel):
    """获取天气的输入参数"""
    city: str = Field(..., description="城市名称，如 '北京'")
    unit: str = Field(default="celsius", description="温度单位")

class Tool(BaseModel):
    """工具定义"""
    name: str
    description: str
    parameters: type[BaseModel]
    function: Callable

    def execute(self, **kwargs) -> Any:
        """执行工具"""
        # 验证参数
        validated_input = self.parameters(**kwargs)
        # 执行函数
        return self.function(**validated_input.dict())
```

### 3.3 完整的工具调用实现

```python
import json
from openai import OpenAI
from typing import List, Dict, Any

class ToolManager:
    """工具管理器"""

    def __init__(self):
        self.tools: Dict[str, Tool] = {}
        self.client = OpenAI()

    def register(self, tool: Tool):
        """注册工具"""
        self.tools[tool.name] = tool

    def get_tool_definitions(self) -> List[dict]:
        """获取所有工具的 JSON Schema 定义"""
        definitions = []
        for tool in self.tools.values():
            definitions.append({
                "type": "function",
                "function": {
                    "name": tool.name,
                    "description": tool.description,
                    "parameters": tool.parameters.schema()
                }
            })
        return definitions

    def execute_tool(self, name: str, arguments: dict) -> str:
        """执行指定工具"""
        if name not in self.tools:
            return f"错误：未找到工具 {name}"

        tool = self.tools[name]
        try:
            result = tool.execute(**arguments)
            return json.dumps(result, ensure_ascii=False)
        except Exception as e:
            return f"工具执行错误：{str(e)}"


class ToolCallingAgent:
    """支持工具调用的 Agent"""

    def __init__(self, tool_manager: ToolManager):
        self.tool_manager = tool_manager
        self.client = OpenAI()
        self.messages: List[dict] = []

    def chat(self, user_message: str) -> str:
        """处理用户消息"""

        # 添加用户消息
        self.messages.append({
            "role": "user",
            "content": user_message
        })

        # 调用 LLM（带工具定义）
        response = self.client.chat.completions.create(
            model="gpt-4",
            messages=self.messages,
            tools=self.tool_manager.get_tool_definitions(),
            tool_choice="auto"  # 让模型自动决定是否使用工具
        )

        assistant_message = response.choices[0].message

        # 检查是否需要调用工具
        if assistant_message.tool_calls:
            # 处理工具调用
            self.messages.append(assistant_message.model_dump())

            for tool_call in assistant_message.tool_calls:
                function_name = tool_call.function.name
                function_args = json.loads(tool_call.function.arguments)

                # 执行工具
                result = self.tool_manager.execute_tool(
                    function_name,
                    function_args
                )

                # 添加工具结果到消息
                self.messages.append({
                    "role": "tool",
                    "tool_call_id": tool_call.id,
                    "content": result
                })

            # 让 LLM 基于工具结果生成最终回复
            final_response = self.client.chat.completions.create(
                model="gpt-4",
                messages=self.messages
            )

            final_content = final_response.choices[0].message.content
            self.messages.append({
                "role": "assistant",
                "content": final_content
            })

            return final_content

        else:
            # 直接回复
            content = assistant_message.content
            self.messages.append({
                "role": "assistant",
                "content": content
            })
            return content
```

### 3.4 使用示例

```python
# 定义具体工具
def get_weather(city: str, unit: str = "celsius") -> dict:
    """模拟天气API调用"""
    # 实际应用中这里会调用真实的天气API
    return {
        "city": city,
        "temperature": 25,
        "unit": unit,
        "condition": "晴朗",
        "humidity": 60
    }

def search_web(query: str, num_results: int = 5) -> list:
    """模拟网络搜索"""
    return [
        {"title": f"搜索结果{i}", "url": f"https://example.com/{i}"}
        for i in range(num_results)
    ]

def calculate(expression: str) -> dict:
    """安全的数学计算"""
    try:
        result = eval(expression, {"__builtins__": {}}, {})
        return {"expression": expression, "result": result}
    except Exception as e:
        return {"error": str(e)}

# 创建工具定义
class WeatherInput(BaseModel):
    city: str = Field(..., description="城市名称")
    unit: str = Field(default="celsius", description="温度单位")

class SearchInput(BaseModel):
    query: str = Field(..., description="搜索关键词")
    num_results: int = Field(default=5, description="返回结果数量")

class CalculateInput(BaseModel):
    expression: str = Field(..., description="数学表达式")

# 注册工具
manager = ToolManager()

manager.register(Tool(
    name="get_weather",
    description="获取指定城市的天气信息",
    parameters=WeatherInput,
    function=get_weather
))

manager.register(Tool(
    name="search_web",
    description="在网络上搜索信息",
    parameters=SearchInput,
    function=search_web
))

manager.register(Tool(
    name="calculate",
    description="执行数学计算",
    parameters=CalculateInput,
    function=calculate
))

# 使用 Agent
agent = ToolCallingAgent(manager)

# 测试
response = agent.chat("北京今天天气怎么样？")
print(response)
# 输出: 北京今天天气晴朗，气温25°C，湿度60%，非常适合外出活动。

response = agent.chat("计算一下 (15 * 8 + 120) / 4 等于多少")
print(response)
# 输出: (15 * 8 + 120) / 4 的计算结果是 60。
```

---

## 4. ReAct 模式：推理与行动

### 4.1 ReAct 概述

```
┌─────────────────────────────────────────────────────────────────────┐
│                    ReAct = Reasoning + Acting                        │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│   传统方法的问题：                                                   │
│   ┌──────────────────────┐     ┌──────────────────────┐            │
│   │ 纯推理 (Chain-of-    │     │ 纯行动 (Direct      │            │
│   │ Thought)              │     │ Action)              │            │
│   │                      │     │                      │            │
│   │ 思考→思考→思考→输出   │     │ 输入→行动→输出       │            │
│   │                      │     │                      │            │
│   │ ❌ 无法获取新信息     │     │ ❌ 缺乏规划和反思    │            │
│   │ ❌ 容易产生幻觉       │     │ ❌ 容易陷入错误循环  │            │
│   └──────────────────────┘     └──────────────────────┘            │
│                                                                     │
│   ReAct 解决方案：                                                  │
│   ┌──────────────────────────────────────────────────────────────┐ │
│   │                                                              │ │
│   │    思考 (Thought) ──→ 行动 (Action) ──→ 观察 (Observation)   │ │
│   │         ↑                                      │             │ │
│   │         └──────────────────────────────────────┘             │ │
│   │                                                              │ │
│   │    ✅ 推理与行动交织                                          │ │
│   │    ✅ 基于真实反馈调整策略                                     │ │
│   │    ✅ 可解释的决策过程                                        │ │
│   │                                                              │ │
│   └──────────────────────────────────────────────────────────────┘ │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 4.2 ReAct 执行示例

```
┌─────────────────────────────────────────────────────────────────────┐
│  问题: "苹果公司的市值和微软相比如何？谁的市值更高？"                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  【第一轮】                                                          │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Thought 1: 我需要先查询苹果公司的当前市值                      │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                               │                                     │
│                               ▼                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Action 1: search("苹果公司当前市值 2024")                     │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                               │                                     │
│                               ▼                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Observation 1: 苹果公司市值约为 2.9 万亿美元                   │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  【第二轮】                                                          │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Thought 2: 我已经知道苹果市值，现在需要查询微软的市值           │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                               │                                     │
│                               ▼                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Action 2: search("微软公司当前市值 2024")                     │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                               │                                     │
│                               ▼                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Observation 2: 微软公司市值约为 3.1 万亿美元                   │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  【第三轮】                                                          │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Thought 3: 我现在有两家公司的市值数据，可以进行比较了          │   │
│  │ 苹果: 2.9万亿, 微软: 3.1万亿，微软市值更高                    │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                               │                                     │
│                               ▼                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Final Answer: 根据最新数据，微软市值(3.1万亿美元)高于          │   │
│  │ 苹果市值(2.9万亿美元)，微软市值更高。                          │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 4.3 ReAct Agent 实现

```python
from dataclasses import dataclass
from enum import Enum
from typing import Optional

class ActionType(Enum):
    SEARCH = "search"
    CALCULATE = "calculate"
    LOOKUP = "lookup"
    FINISH = "finish"

@dataclass
class ReActStep:
    """ReAct 单步"""
    thought: str
    action_type: ActionType
    action_input: str
    observation: Optional[str] = None

class ReActAgent:
    """ReAct Agent 实现"""

    REACT_PROMPT = """
你是一个智能助手，使用 ReAct 方法解决问题。

可用的工具：
- search(query): 搜索互联网获取信息
- calculate(expression): 执行数学计算
- lookup(term): 在知识库中查找术语定义
- finish(answer): 给出最终答案

请按照以下格式回答：

Thought: [你的推理过程]
Action: [工具名称]
Action Input: [工具参数]

等待观察结果后继续推理，直到可以给出最终答案。
当你确定答案后，使用 finish 给出最终答案。

问题：{question}

{agent_scratchpad}
"""

    def __init__(self, tools: Dict[str, Callable]):
        self.tools = tools
        self.client = OpenAI()
        self.steps: List[ReActStep] = []

    def parse_action(self, response: str) -> tuple:
        """解析 LLM 输出的动作"""
        import re

        # 提取 Thought
        thought_match = re.search(r"Thought:\s*(.+?)(?=\nAction:)", response, re.DOTALL)
        thought = thought_match.group(1).strip() if thought_match else ""

        # 提取 Action
        action_match = re.search(r"Action:\s*(\w+)", response)
        action = action_match.group(1) if action_match else "finish"

        # 提取 Action Input
        input_match = re.search(r"Action Input:\s*(.+?)(?=\n|$)", response, re.DOTALL)
        action_input = input_match.group(1).strip() if input_match else ""

        return thought, action, action_input

    def format_scratchpad(self) -> str:
        """格式化已有的推理步骤"""
        scratchpad = ""
        for step in self.steps:
            scratchpad += f"Thought: {step.thought}\n"
            scratchpad += f"Action: {step.action_type.value}\n"
            scratchpad += f"Action Input: {step.action_input}\n"
            if step.observation:
                scratchpad += f"Observation: {step.observation}\n"
        return scratchpad

    def run(self, question: str, max_steps: int = 10) -> str:
        """运行 ReAct 循环"""

        self.steps = []

        for _ in range(max_steps):
            # 构建提示词
            prompt = self.REACT_PROMPT.format(
                question=question,
                agent_scratchpad=self.format_scratchpad()
            )

            # 调用 LLM
            response = self.client.chat.completions.create(
                model="gpt-4",
                messages=[{"role": "user", "content": prompt}],
                temperature=0
            )

            output = response.choices[0].message.content
            thought, action, action_input = self.parse_action(output)

            # 检查是否完成
            if action.lower() == "finish":
                return action_input

            # 执行工具
            if action.lower() in self.tools:
                observation = self.tools[action.lower()](action_input)
            else:
                observation = f"未知工具: {action}"

            # 记录步骤
            self.steps.append(ReActStep(
                thought=thought,
                action_type=ActionType(action.lower()),
                action_input=action_input,
                observation=str(observation)
            ))

        return "达到最大步骤限制，未能得出结论"

# 使用示例
tools = {
    "search": lambda q: f"搜索结果: {q}的相关信息...",
    "calculate": lambda expr: str(eval(expr)),
    "lookup": lambda term: f"{term}的定义是..."
}

agent = ReActAgent(tools)
answer = agent.run("2023年中国GDP是多少？换算成欧元是多少？")
print(answer)
```

---

## 5. MCP 协议详解

### 5.1 什么是 MCP？

```
┌─────────────────────────────────────────────────────────────────────┐
│                   MCP (Model Context Protocol)                       │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│   MCP 是由 Anthropic 提出的开放协议，用于标准化 LLM 应用与外部         │
│   数据源和工具之间的连接方式。                                        │
│                                                                     │
│   类比理解：                                                         │
│   ┌─────────────────────────────────────────────────────────────┐  │
│   │                                                             │  │
│   │   USB 之于 硬件设备  =  MCP 之于 AI 应用                     │  │
│   │                                                             │  │
│   │   就像 USB 让各种设备用统一接口连接电脑一样，                   │  │
│   │   MCP 让各种数据源和工具用统一协议连接 AI 应用                  │  │
│   │                                                             │  │
│   └─────────────────────────────────────────────────────────────┘  │
│                                                                     │
│   核心价值：                                                         │
│   ┌────────────┐  ┌────────────┐  ┌────────────┐                   │
│   │  标准化    │  │  可复用    │  │  安全      │                   │
│   │  统一协议  │  │  插件生态  │  │  权限控制  │                   │
│   └────────────┘  └────────────┘  └────────────┘                   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 5.2 MCP 架构

```
┌─────────────────────────────────────────────────────────────────────┐
│                         MCP 架构图                                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│   ┌──────────────────────────────────────────────────────────────┐ │
│   │                      MCP Hosts (宿主)                         │ │
│   │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐          │ │
│   │  │  Claude     │  │   IDE       │  │   Custom    │          │ │
│   │  │  Desktop    │  │  (Cursor)   │  │   App       │          │ │
│   │  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘          │ │
│   └─────────┼────────────────┼────────────────┼──────────────────┘ │
│             │                │                │                     │
│             └────────────────┼────────────────┘                     │
│                              │                                      │
│                              ▼                                      │
│   ┌──────────────────────────────────────────────────────────────┐ │
│   │                    MCP Client (客户端)                        │ │
│   │         负责维护与 Server 的连接，处理协议通信                  │ │
│   └──────────────────────────────────────────────────────────────┘ │
│                              │                                      │
│               ┌──────────────┼──────────────┐                       │
│               │              │              │                       │
│               ▼              ▼              ▼                       │
│   ┌──────────────────────────────────────────────────────────────┐ │
│   │                   MCP Servers (服务端)                        │ │
│   │  ┌───────────┐  ┌───────────┐  ┌───────────┐  ┌───────────┐ │ │
│   │  │ Database  │  │   File    │  │   API     │  │  Custom   │ │ │
│   │  │  Server   │  │  System   │  │  Server   │  │  Server   │ │ │
│   │  │           │  │  Server   │  │           │  │           │ │ │
│   │  └─────┬─────┘  └─────┬─────┘  └─────┬─────┘  └─────┬─────┘ │ │
│   └────────┼──────────────┼──────────────┼──────────────┼────────┘ │
│            │              │              │              │          │
│            ▼              ▼              ▼              ▼          │
│   ┌──────────────────────────────────────────────────────────────┐ │
│   │                     数据源 / 服务                             │ │
│   │  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────────────┐ │ │
│   │  │ MySQL   │  │ 本地    │  │ GitHub  │  │ Slack/邮件/...  │ │ │
│   │  │ MongoDB │  │ 文件    │  │ GitLab  │  │                 │ │ │
│   │  └─────────┘  └─────────┘  └─────────┘  └─────────────────┘ │ │
│   └──────────────────────────────────────────────────────────────┘ │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 5.3 MCP 核心概念

```
┌─────────────────────────────────────────────────────────────────────┐
│                      MCP 三大核心原语                                │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  1. Resources (资源)                                         │   │
│  │     • 向 LLM 暴露数据和内容                                    │   │
│  │     • 类似于 GET 请求，用于读取数据                            │   │
│  │     • 例如：文件内容、数据库记录、API 响应                      │   │
│  │                                                              │   │
│  │     URI格式: file:///path/to/file                            │   │
│  │              db://database/table/record                      │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  2. Tools (工具)                                             │   │
│  │     • 向 LLM 暴露可执行的功能                                  │   │
│  │     • 类似于 POST 请求，用于执行操作                           │   │
│  │     • 例如：发送邮件、创建文件、执行查询                        │   │
│  │                                                              │   │
│  │     定义: name + description + inputSchema                   │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  3. Prompts (提示词模板)                                      │   │
│  │     • 预定义的提示词模板                                       │   │
│  │     • 可复用的交互模式                                        │   │
│  │     • 例如：代码审查模板、数据分析模板                          │   │
│  │                                                              │   │
│  │     支持: 参数化 + 动态内容                                    │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 5.4 MCP Server 实现

````python
# mcp_server.py
from mcp.server import Server
from mcp.server.stdio import stdio_server
from mcp.types import (
    Resource,
    Tool,
    TextContent,
    INVALID_PARAMS,
    INTERNAL_ERROR
)
import json
import asyncio

# 创建 MCP Server 实例
server = Server("my-mcp-server")

# ==================== Resources ====================

@server.list_resources()
async def list_resources():
    """列出可用资源"""
    return [
        Resource(
            uri="file:///config/settings.json",
            name="应用配置",
            description="应用程序的配置文件",
            mimeType="application/json"
        ),
        Resource(
            uri="db://users/profile",
            name="用户资料",
            description="当前用户的资料信息",
            mimeType="application/json"
        )
    ]

@server.read_resource()
async def read_resource(uri: str):
    """读取资源内容"""
    if uri == "file:///config/settings.json":
        config = {
            "app_name": "My Application",
            "version": "1.0.0",
            "debug": True
        }
        return json.dumps(config, indent=2)

    elif uri == "db://users/profile":
        profile = {
            "username": "john_doe",
            "email": "john@example.com",
            "role": "admin"
        }
        return json.dumps(profile, indent=2)

    raise ValueError(f"未知资源: {uri}")

# ==================== Tools ====================

@server.list_tools()
async def list_tools():
    """列出可用工具"""
    return [
        Tool(
            name="send_email",
            description="发送电子邮件",
            inputSchema={
                "type": "object",
                "properties": {
                    "to": {
                        "type": "string",
                        "description": "收件人邮箱地址"
                    },
                    "subject": {
                        "type": "string",
                        "description": "邮件主题"
                    },
                    "body": {
                        "type": "string",
                        "description": "邮件正文"
                    }
                },
                "required": ["to", "subject", "body"]
            }
        ),
        Tool(
            name="query_database",
            description="执行数据库查询",
            inputSchema={
                "type": "object",
                "properties": {
                    "sql": {
                        "type": "string",
                        "description": "SQL 查询语句"
                    },
                    "database": {
                        "type": "string",
                        "description": "数据库名称"
                    }
                },
                "required": ["sql"]
            }
        ),
        Tool(
            name="create_file",
            description="创建新文件",
            inputSchema={
                "type": "object",
                "properties": {
                    "path": {
                        "type": "string",
                        "description": "文件路径"
                    },
                    "content": {
                        "type": "string",
                        "description": "文件内容"
                    }
                },
                "required": ["path", "content"]
            }
        )
    ]

@server.call_tool()
async def call_tool(name: str, arguments: dict):
    """执行工具"""

    if name == "send_email":
        # 模拟发送邮件
        to = arguments.get("to")
        subject = arguments.get("subject")
        body = arguments.get("body")

        # 实际实现中这里会调用邮件服务
        return [TextContent(
            type="text",
            text=f"邮件已发送至 {to}\n主题: {subject}"
        )]

    elif name == "query_database":
        sql = arguments.get("sql")
        database = arguments.get("database", "default")

        # 模拟数据库查询
        mock_result = [
            {"id": 1, "name": "Alice", "age": 30},
            {"id": 2, "name": "Bob", "age": 25}
        ]

        return [TextContent(
            type="text",
            text=json.dumps(mock_result, indent=2)
        )]

    elif name == "create_file":
        path = arguments.get("path")
        content = arguments.get("content")

        # 实际实现中这里会创建文件
        return [TextContent(
            type="text",
            text=f"文件已创建: {path}"
        )]

    raise ValueError(f"未知工具: {name}")

# ==================== Prompts ====================

@server.list_prompts()
async def list_prompts():
    """列出可用提示词模板"""
    from mcp.types import Prompt, PromptArgument

    return [
        Prompt(
            name="code_review",
            description="代码审查提示词模板",
            arguments=[
                PromptArgument(
                    name="code",
                    description="需要审查的代码",
                    required=True
                ),
                PromptArgument(
                    name="language",
                    description="编程语言",
                    required=False
                )
            ]
        ),
        Prompt(
            name="summarize",
            description="文本总结提示词模板",
            arguments=[
                PromptArgument(
                    name="text",
                    description="需要总结的文本",
                    required=True
                ),
                PromptArgument(
                    name="max_length",
                    description="最大长度",
                    required=False
                )
            ]
        )
    ]

@server.get_prompt()
async def get_prompt(name: str, arguments: dict):
    """获取提示词内容"""
    from mcp.types import PromptMessage, TextContent

    if name == "code_review":
        code = arguments.get("code", "")
        language = arguments.get("language", "unknown")

        return {
            "messages": [
                PromptMessage(
                    role="user",
                    content=TextContent(
                        type="text",
                        text=f"""请对以下 {language} 代码进行审查：

```{language}
{code}
````

请从以下方面进行评估：

1.  代码质量和可读性
2.  潜在的 Bug 或问题
3.  性能优化建议
4.  最佳实践遵循情况
    """
    )
    )
    ]
    }
        raise ValueError(f"未知提示词模板: {name}")

# 启动服务器

async def main():
async with stdio_server() as (read_stream, write_stream):
await server.run(
read_stream,
write_stream,
server.create_initialization_options()
)

if **name** == "**main**":
asyncio.run(main())

````

### 5.5 MCP Client 实现

```python
# mcp_client.py
from mcp import ClientSession, StdioServerParameters
from mcp.client.stdio import stdio_client
import asyncio
import json

class MCPClient:
    """MCP 客户端封装"""

    def __init__(self):
        self.session: ClientSession = None
        self.tools = []
        self.resources = []

    async def connect(self, server_command: str, args: list = None):
        """连接到 MCP Server"""
        server_params = StdioServerParameters(
            command=server_command,
            args=args or []
        )

        transport = await stdio_client(server_params)
        self.read, self.write = transport
        self.session = ClientSession(self.read, self.write)

        # 初始化连接
        await self.session.initialize()

        # 获取可用工具和资源
        await self._fetch_capabilities()

    async def _fetch_capabilities(self):
        """获取服务器能力"""
        # 获取工具列表
        tools_response = await self.session.list_tools()
        self.tools = tools_response.tools

        # 获取资源列表
        resources_response = await self.session.list_resources()
        self.resources = resources_response.resources

    async def call_tool(self, name: str, arguments: dict) -> str:
        """调用工具"""
        result = await self.session.call_tool(name, arguments)

        # 提取文本内容
        texts = []
        for content in result.content:
            if hasattr(content, 'text'):
                texts.append(content.text)

        return "\n".join(texts)

    async def read_resource(self, uri: str) -> str:
        """读取资源"""
        result = await self.session.read_resource(uri)

        texts = []
        for content in result.contents:
            if hasattr(content, 'text'):
                texts.append(content.text)

        return "\n".join(texts)

    def get_tools_for_llm(self) -> list:
        """获取工具定义供 LLM 使用"""
        return [
            {
                "type": "function",
                "function": {
                    "name": tool.name,
                    "description": tool.description,
                    "parameters": tool.inputSchema
                }
            }
            for tool in self.tools
        ]

    async def close(self):
        """关闭连接"""
        if self.session:
            await self.session.close()


# 集成到 Agent
class MCPAgent:
    """基于 MCP 的 Agent"""

    def __init__(self):
        self.mcp_client = MCPClient()
        self.client = OpenAI()
        self.messages = []

    async def setup(self, server_configs: list):
        """设置 MCP 连接"""
        for config in server_configs:
            await self.mcp_client.connect(
                config["command"],
                config.get("args", [])
            )

    async def chat(self, user_message: str) -> str:
        """处理用户消息"""
        self.messages.append({
            "role": "user",
            "content": user_message
        })

        # 获取 MCP 工具定义
        tools = self.mcp_client.get_tools_for_llm()

        # 调用 LLM
        response = self.client.chat.completions.create(
            model="gpt-4",
            messages=self.messages,
            tools=tools,
            tool_choice="auto"
        )

        assistant_message = response.choices[0].message

        if assistant_message.tool_calls:
            self.messages.append(assistant_message.model_dump())

            for tool_call in assistant_message.tool_calls:
                # 通过 MCP 调用工具
                result = await self.mcp_client.call_tool(
                    tool_call.function.name,
                    json.loads(tool_call.function.arguments)
                )

                self.messages.append({
                    "role": "tool",
                    "tool_call_id": tool_call.id,
                    "content": result
                })

            # 生成最终回复
            final_response = self.client.chat.completions.create(
                model="gpt-4",
                messages=self.messages
            )

            return final_response.choices[0].message.content

        return assistant_message.content


# 使用示例
async def main():
    agent = MCPAgent()

    # 连接 MCP 服务器
    await agent.setup([
        {
            "command": "python",
            "args": ["mcp_server.py"]
        }
    ])

    # 对话
    response = await agent.chat("查询一下用户资料")
    print(response)

    response = await agent.chat("给 test@example.com 发一封邮件，主题是测试")
    print(response)

if __name__ == "__main__":
    asyncio.run(main())
````

### 5.6 MCP 配置文件

```json
// claude_desktop_config.json (Claude Desktop 配置示例)
{
  "mcpServers": {
    "filesystem": {
      "command": "npx",
      "args": [
        "-y",
        "@modelcontextprotocol/server-filesystem",
        "/path/to/allowed/directory"
      ]
    },
    "database": {
      "command": "python",
      "args": ["./my_db_server.py"],
      "env": {
        "DATABASE_URL": "postgresql://localhost/mydb"
      }
    },
    "github": {
      "command": "npx",
      "args": ["-y", "@modelcontextprotocol/server-github"],
      "env": {
        "GITHUB_TOKEN": "your-github-token"
      }
    }
  }
}
```

---

## 6. 实战：构建完整 Agent

### 6.1 项目结构

```
my_agent/
├── agent/
│   ├── __init__.py
│   ├── core.py          # Agent 核心逻辑
│   ├── memory.py        # 记忆系统
│   ├── planning.py      # 规划模块
│   └── tools/           # 工具目录
│       ├── __init__.py
│       ├── base.py      # 工具基类
│       ├── search.py    # 搜索工具
│       ├── calculator.py # 计算工具
│       └── file_ops.py  # 文件操作
├── mcp/
│   ├── server.py        # MCP 服务器
│   └── client.py        # MCP 客户端
├── config/
│   └── settings.py      # 配置
├── main.py              # 入口
└── requirements.txt
```

### 6.2 完整实现

```python
# agent/tools/base.py
from abc import ABC, abstractmethod
from pydantic import BaseModel
from typing import Any, Type

class BaseTool(ABC):
    """工具基类"""

    name: str
    description: str
    args_schema: Type[BaseModel]

    @abstractmethod
    def run(self, **kwargs) -> Any:
        """执行工具"""
        pass

    def to_openai_schema(self) -> dict:
        """转换为 OpenAI 工具格式"""
        return {
            "type": "function",
            "function": {
                "name": self.name,
                "description": self.description,
                "parameters": self.args_schema.model_json_schema()
            }
        }


# agent/tools/search.py
import httpx
from pydantic import BaseModel, Field

class SearchInput(BaseModel):
    query: str = Field(..., description="搜索关键词")
    num_results: int = Field(default=5, description="返回结果数量")

class SearchTool(BaseTool):
    name = "web_search"
    description = "在网络上搜索信息，适用于需要获取最新信息的场景"
    args_schema = SearchInput

    def __init__(self, api_key: str):
        self.api_key = api_key

    def run(self, query: str, num_results: int = 5) -> list:
        # 使用搜索 API (示例用 Serper)
        response = httpx.post(
            "https://google.serper.dev/search",
            headers={"X-API-KEY": self.api_key},
            json={"q": query, "num": num_results}
        )
        results = response.json().get("organic", [])
        return [
            {"title": r["title"], "snippet": r["snippet"], "link": r["link"]}
            for r in results
        ]


# agent/memory.py
from typing import List, Dict, Any, Optional
from dataclasses import dataclass, field
from datetime import datetime
import json

@dataclass
class Message:
    role: str
    content: str
    timestamp: datetime = field(default_factory=datetime.now)
    metadata: Dict[str, Any] = field(default_factory=dict)

class ConversationMemory:
    """对话记忆"""

    def __init__(self, max_messages: int = 50):
        self.messages: List[Message] = []
        self.max_messages = max_messages

    def add(self, role: str, content: str, **metadata):
        """添加消息"""
        self.messages.append(Message(
            role=role,
            content=content,
            metadata=metadata
        ))

        # 超过限制时压缩
        if len(self.messages) > self.max_messages:
            self._compress()

    def _compress(self):
        """压缩旧消息"""
        # 保留最近的消息，压缩旧消息为摘要
        old_messages = self.messages[:-10]
        recent_messages = self.messages[-10:]

        # 创建摘要
        summary = self._summarize(old_messages)

        self.messages = [
            Message(role="system", content=f"[历史摘要]: {summary}")
        ] + recent_messages

    def _summarize(self, messages: List[Message]) -> str:
        """生成消息摘要"""
        # 简单实现，实际可调用 LLM 生成
        return f"过去的 {len(messages)} 条消息的摘要..."

    def get_context(self) -> List[dict]:
        """获取上下文"""
        return [
            {"role": m.role, "content": m.content}
            for m in self.messages
        ]

    def clear(self):
        """清空记忆"""
        self.messages = []


# agent/core.py
from openai import OpenAI
from typing import List, Dict, Any, Optional
import json

class Agent:
    """完整的 Agent 实现"""

    def __init__(
        self,
        model: str = "gpt-4",
        system_prompt: Optional[str] = None,
        tools: Optional[List[BaseTool]] = None,
        max_iterations: int = 10,
        verbose: bool = True
    ):
        self.model = model
        self.client = OpenAI()
        self.memory = ConversationMemory()
        self.tools = {t.name: t for t in (tools or [])}
        self.max_iterations = max_iterations
        self.verbose = verbose

        # 系统提示词
        self.system_prompt = system_prompt or """
你是一个智能助手，能够使用各种工具来帮助用户完成任务。

在处理用户请求时，请遵循以下原则：
1. 仔细分析用户需求
2. 选择最合适的工具
3. 如果一个工具不够，可以组合多个工具
4. 始终基于真实数据给出回答
5. 如果不确定，坦诚告知用户
"""

        # 添加系统消息
        self.memory.add("system", self.system_prompt)

    def _get_tool_schemas(self) -> List[dict]:
        """获取工具 Schema"""
        return [tool.to_openai_schema() for tool in self.tools.values()]

    def _execute_tool(self, name: str, arguments: dict) -> str:
        """执行工具"""
        if name not in self.tools:
            return f"错误：未找到工具 '{name}'"

        tool = self.tools[name]
        try:
            result = tool.run(**arguments)
            return json.dumps(result, ensure_ascii=False, indent=2)
        except Exception as e:
            return f"工具执行错误：{str(e)}"

    def _log(self, message: str):
        """输出日志"""
        if self.verbose:
            print(f"[Agent] {message}")

    def chat(self, user_input: str) -> str:
        """处理用户输入"""

        self._log(f"收到用户输入: {user_input}")
        self.memory.add("user", user_input)

        for iteration in range(self.max_iterations):
            self._log(f"迭代 {iteration + 1}/{self.max_iterations}")

            # 构建消息
            messages = self.memory.get_context()

            # 调用 LLM
            response = self.client.chat.completions.create(
                model=self.model,
                messages=messages,
                tools=self._get_tool_schemas() if self.tools else None,
                tool_choice="auto" if self.tools else None,
                temperature=0.7
            )

            assistant_message = response.choices[0].message

            # 检查是否需要调用工具
            if assistant_message.tool_calls:
                self._log(f"需要调用 {len(assistant_message.tool_calls)} 个工具")

                # 添加助手消息
                self.memory.add(
                    "assistant",
                    assistant_message.content or "",
                    tool_calls=[tc.model_dump() for tc in assistant_message.tool_calls]
                )

                # 执行每个工具调用
                for tool_call in assistant_message.tool_calls:
                    func_name = tool_call.function.name
                    func_args = json.loads(tool_call.function.arguments)

                    self._log(f"调用工具: {func_name}({func_args})")

                    result = self._execute_tool(func_name, func_args)

                    self._log(f"工具结果: {result[:100]}...")

                    # 添加工具结果
                    self.memory.add(
                        "tool",
                        result,
                        tool_call_id=tool_call.id,
                        name=func_name
                    )

            else:
                # 没有工具调用，返回最终答案
                final_response = assistant_message.content
                self.memory.add("assistant", final_response)

                self._log(f"最终回复: {final_response[:100]}...")
                return final_response

        return "抱歉，我无法在规定的步骤内完成任务。请尝试简化您的请求。"

    def reset(self):
        """重置对话"""
        self.memory.clear()
        self.memory.add("system", self.system_prompt)


# main.py
from agent.core import Agent
from agent.tools.search import SearchTool
from agent.tools.calculator import CalculatorTool

def main():
    # 创建工具
    tools = [
        SearchTool(api_key="your-api-key"),
        CalculatorTool()
    ]

    # 创建 Agent
    agent = Agent(
        model="gpt-4",
        tools=tools,
        verbose=True
    )

    # 交互循环
    print("Agent 已启动，输入 'quit' 退出")
    print("-" * 50)

    while True:
        user_input = input("\n你: ").strip()

        if user_input.lower() in ['quit', 'exit', 'q']:
            print("再见！")
            break

        if not user_input:
            continue

        response = agent.chat(user_input)
        print(f"\nAgent: {response}")

if __name__ == "__main__":
    main()
```

### 6.3 运行效果

```
Agent 已启动，输入 'quit' 退出
--------------------------------------------------

你: 帮我查一下今天的科技新闻，然后计算一下如果苹果股价200美元，买100股需要多少钱

[Agent] 收到用户输入: 帮我查一下今天的科技新闻...
[Agent] 迭代 1/10
[Agent] 需要调用 2 个工具
[Agent] 调用工具: web_search({"query": "今日科技新闻", "num_results": 5})
[Agent] 工具结果: [{"title": "苹果发布新款MacBook...", ...}]
[Agent] 调用工具: calculator({"expression": "200 * 100"})
[Agent] 工具结果: {"result": 20000}
[Agent] 迭代 2/10
[Agent] 最终回复: 根据我搜索到的今日科技新闻...

Agent:
## 今日科技新闻摘要

1. **苹果发布新款 MacBook Pro** - 搭载 M3 芯片...
2. **Google 推出 Gemini 模型** - 多模态能力大幅提升...
3. **特斯拉 Cybertruck 开始交付** - 首批车主已收到...

## 股票计算

如果苹果股价为 **200 美元**，购买 **100 股**需要：
- 总金额：**$20,000 美元**

需要我帮您查询苹果当前的实际股价吗？
```

---

## 7. 最佳实践与常见陷阱

### 7.1 最佳实践

```
┌─────────────────────────────────────────────────────────────────────┐
│                         Agent 开发最佳实践                           │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  📌 工具设计                                                        │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ ✅ DO                          │ ❌ DON'T                   │   │
│  │ • 工具功能单一明确              │ • 工具功能过于复杂          │   │
│  │ • 清晰的参数描述               │ • 模糊的描述               │   │
│  │ • 完善的错误处理               │ • 吞掉异常                 │   │
│  │ • 返回结构化数据               │ • 返回大量原始数据          │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  📌 提示词工程                                                       │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ ✅ DO                          │ ❌ DON'T                   │   │
│  │ • 提供清晰的角色定义            │ • 模糊的指令               │   │
│  │ • 包含使用工具的示例            │ • 假设模型会理解意图        │   │
│  │ • 设定边界和限制               │ • 给予无限制的权限          │   │
│  │ • 定义输出格式                 │ • 期望模型猜测格式          │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  📌 错误处理                                                        │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ ✅ DO                          │ ❌ DON'T                   │   │
│  │ • 设置最大迭代次数             │ • 无限循环风险             │   │
│  │ • 工具调用超时                 │ • 阻塞等待                 │   │
│  │ • 优雅降级                     │ • 直接崩溃                 │   │
│  │ • 详细的日志记录               │ • 黑盒运行                 │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 7.2 常见陷阱与解决方案

```python
# ❌ 陷阱1：工具描述不清晰
bad_tool = {
    "name": "search",
    "description": "搜索"  # 太简短
}

# ✅ 改进
good_tool = {
    "name": "web_search",
    "description": "在互联网上搜索信息。适用于需要查找最新新闻、事实、"
                   "统计数据等场景。不适用于数学计算或代码执行。"
}

# ❌ 陷阱2：无限循环
def bad_agent_loop():
    while True:  # 危险：可能永远不会停止
        response = llm.chat()
        if response.tool_calls:
            execute_tools()

# ✅ 改进
def good_agent_loop(max_iterations=10):
    for i in range(max_iterations):
        response = llm.chat()
        if not response.tool_calls:
            return response.content
        execute_tools()
    return "达到最大迭代次数"

# ❌ 陷阱3：未验证工具输入
def bad_tool_execution(args: dict):
    path = args["path"]
    os.remove(path)  # 危险：可能删除重要文件

# ✅ 改进
def good_tool_execution(args: dict):
    path = args["path"]

    # 验证路径
    allowed_paths = ["/tmp/", "/home/user/workspace/"]
    if not any(path.startswith(p) for p in allowed_paths):
        return {"error": "不允许访问该路径"}

    # 确认文件存在
    if not os.path.exists(path):
        return {"error": "文件不存在"}

    os.remove(path)
    return {"success": True}

# ❌ 陷阱4：Token 使用失控
def bad_memory_management():
    messages = []
    for msg in conversation:
        messages.append(msg)  # 永远增长

    response = llm.chat(messages)  # 可能超出 token 限制

# ✅ 改进
def good_memory_management(max_tokens=4000):
    messages = []
    total_tokens = 0

    for msg in reversed(conversation):
        msg_tokens = count_tokens(msg)
        if total_tokens + msg_tokens > max_tokens:
            break
        messages.insert(0, msg)
        total_tokens += msg_tokens

    response = llm.chat(messages)
```

### 7.3 安全考虑

```
┌─────────────────────────────────────────────────────────────────────┐
│                          安全考虑清单                                │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  🔒 输入验证                                                        │
│     □ 验证所有用户输入                                              │
│     □ 验证工具参数类型和范围                                         │
│     □ 防止注入攻击（SQL、命令等）                                    │
│                                                                     │
│  🔒 权限控制                                                        │
│     □ 最小权限原则                                                  │
│     □ 工具访问白名单                                                │
│     □ 敏感操作需二次确认                                            │
│                                                                     │
│  🔒 资源限制                                                        │
│     □ API 调用频率限制                                              │
│     □ Token 使用上限                                                │
│     □ 执行时间超时                                                  │
│                                                                     │
│  🔒 审计日志                                                        │
│     □ 记录所有工具调用                                              │
│     □ 记录用户输入和系统输出                                         │
│     □ 可追溯的操作链路                                              │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

---

## 8. 总结

### 8.1 核心概念回顾

```
┌─────────────────────────────────────────────────────────────────────┐
│                          核心概念回顾                                │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│   Agent                                                             │
│   ├── 定义：LLM + 记忆 + 工具 + 规划                                │
│   ├── 核心循环：感知 → 思考 → 决策 → 执行 → 观察                     │
│   └── 关键能力：自主决策、工具使用、任务规划                          │
│                                                                     │
│   工具调用                                                          │
│   ├── 机制：LLM 生成工具调用请求 → 系统执行 → 结果返回 LLM           │
│   ├── 定义：JSON Schema 描述工具名称、描述、参数                     │
│   └── 关键：清晰的描述帮助 LLM 正确选择和使用工具                     │
│                                                                     │
│   ReAct                                                             │
│   ├── 含义：Reasoning + Acting 交织                                 │
│   ├── 流程：Thought → Action → Observation → 循环                   │
│   └── 优势：可解释性强、能基于反馈调整策略                            │
│                                                                     │
│   MCP                                                               │
│   ├── 定义：Model Context Protocol，标准化连接协议                   │
│   ├── 组件：Host、Client、Server                                    │
│   ├── 原语：Resources、Tools、Prompts                               │
│   └── 价值：标准化、可复用、安全                                     │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 8.2 学习路径建议

```
┌─────────────────────────────────────────────────────────────────────┐
│                          下一步学习建议                              │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│   初级 ──────────────────────────────────────────────────           │
│   ✅ 理解 Agent 基本概念                                            │
│   ✅ 实现简单的工具调用                                             │
│   ✅ 掌握 ReAct 模式                                                │
│                                                                     │
│   中级 ──────────────────────────────────────────────────           │
│   □ 实现完整的 Agent 系统                                           │
│   □ 集成 MCP 协议                                                   │
│   □ 实现复杂的记忆系统                                              │
│                                                                     │
│   高级 ──────────────────────────────────────────────────           │
│   □ 多 Agent 协作系统                                               │
│   □ 自我反思与改进                                                  │
│   □ 复杂任务规划与分解                                              │
│   □ 生产环境部署与优化                                              │
│                                                                     │
│   推荐资源：                                                        │
│   • LangChain / LangGraph 框架                                      │
│   • AutoGPT / MetaGPT 开源项目                                      │
│   • MCP 官方文档                                                    │
│   • OpenAI Cookbook                                                 │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

---

**文档版本**: 1.0  
**最后更新**: 2024年1月  
**作者**: AI Learning Assistant
