"""
Agent 基础概念
=============

学习目标：
    1. 理解什么是 Agent
    2. 掌握 Agent 的核心组件
    3. 理解 Agent 执行循环

核心概念：
    - Agent：LLM + 记忆 + 工具 + 规划
    - 执行循环：感知 → 思考 → 决策 → 执行 → 观察
    - Agent vs 传统 LLM

前置知识：
    - Phase 1-5 的所有内容

环境要求：
    - pip install openai python-dotenv
"""

import os
from dotenv import load_dotenv

load_dotenv()


# ==================== 第一部分：什么是 Agent ====================


def what_is_agent():
    """什么是 Agent"""
    print("=" * 60)
    print("第一部分：什么是 Agent")
    print("=" * 60)

    print("""
    Agent（智能体）的定义
    ─────────────────────
    
    Agent = LLM + 记忆 + 工具 + 规划能力
    
    ┌─────────────────────────────────────────────────────┐
    │                    Agent 架构                        │
    │                                                     │
    │  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌────────┐ │
    │  │   LLM   │  │  Memory │  │  Tools  │  │Planning│ │
    │  │  大脑   │  │  记忆   │  │  工具   │  │  规划  │ │
    │  └─────────┘  └─────────┘  └─────────┘  └────────┘ │
    │                                                     │
    │  Agent 是能够自主感知环境、做出决策、执行行动的系统    │
    └─────────────────────────────────────────────────────┘
    
    Agent 的核心能力：
    ─────────────────
    🧠 理解：分析用户意图和上下文
    🤔 规划：分解任务，制定执行计划
    🔧 行动：调用工具执行具体操作
    🔄 迭代：根据反馈调整策略
    """)


# ==================== 第二部分：Agent vs 传统 LLM ====================


def agent_vs_llm():
    """Agent 与传统 LLM 对比"""
    print("\n" + "=" * 60)
    print("第二部分：Agent vs 传统 LLM")
    print("=" * 60)

    print("""
    传统 LLM 的局限性
    ─────────────────
    
    用户: "今天北京天气怎么样？"
    
    传统 LLM 回复:
    ❌ "我无法获取实时天气信息，我的知识截止于..."
    
    Agent 回复:
    ✅ "让我查询一下...根据查询，今天北京晴朗，25°C。"
    
    ┌────────────────────┬────────────────────────────────┐
    │     传统 LLM       │           Agent                │
    ├────────────────────┼────────────────────────────────┤
    │ 单轮对话           │ 多轮推理循环                    │
    │ 静态知识           │ 动态获取信息                    │
    │ 无法执行操作       │ 可执行真实操作                  │
    │ 被动响应           │ 主动完成任务                    │
    │ 知识有截止日期     │ 通过工具获取实时信息            │
    └────────────────────┴────────────────────────────────┘
    """)


# ==================== 第三部分：Agent 核心组件 ====================


def agent_components():
    """Agent 核心组件"""
    print("\n" + "=" * 60)
    print("第三部分：Agent 核心组件")
    print("=" * 60)

    print("""
    1. LLM（大脑）
    ──────────────
    - 理解用户输入
    - 推理和决策
    - 生成回复
    
    2. 记忆系统
    ──────────────
    - 短期记忆：当前对话历史
    - 长期记忆：向量数据库存储
    - 工作记忆：当前任务状态
    
    3. 工具集合
    ──────────────
    - 搜索工具：获取实时信息
    - 计算工具：执行数学运算
    - API 工具：与外部服务交互
    - 代码执行器：运行代码
    
    4. 规划模块
    ──────────────
    - 任务分解：将复杂任务拆分
    - 步骤排序：确定执行顺序
    - 资源分配：选择合适工具
    """)

    # 组件代码示例
    print("\n📌 组件代码示例：")

    code_example = '''
class AgentComponent:
    """Agent 核心组件抽象"""
    
    def __init__(self):
        # 1. LLM 大脑
        self.llm = None  # GPT-4, Claude 等
        
        # 2. 记忆系统
        self.short_term_memory = []  # 对话历史
        self.long_term_memory = None  # 向量数据库
        
        # 3. 工具集合
        self.tools = {}  # 可用工具字典
        
        # 4. 规划模块
        self.planner = None  # 任务规划器
'''
    print(code_example)


# ==================== 第四部分：Agent 执行循环 ====================


def agent_loop():
    """Agent 执行循环"""
    print("\n" + "=" * 60)
    print("第四部分：Agent 执行循环")
    print("=" * 60)

    print("""
    Agent 执行循环 (Agent Loop)
    ─────────────────────────────
    
              ┌──────────────────────────────┐
              │                              │
              ▼                              │
        ┌─────────────┐                      │
        │   感知      │  接收输入             │
        │  Perceive   │                      │
        └──────┬──────┘                      │
               │                             │
               ▼                             │
        ┌─────────────┐                      │
        │   思考      │  LLM 推理            │
        │   Think     │                      │
        └──────┬──────┘                      │
               │                             │
               ▼                             │
        ┌─────────────┐      ┌──────────┐   │
        │   决策      │──是──│需要工具？│   │
        │  Decide     │      └────┬─────┘   │
        └──────┬──────┘           │否       │
               │                  ▼         │
               │           ┌──────────┐     │
               │           │ 直接回复 │─────┤
               ▼           └──────────┘     │
        ┌─────────────┐                     │
        │   执行      │  调用工具            │
        │    Act      │                     │
        └──────┬──────┘                     │
               │                            │
               ▼                            │
        ┌─────────────┐      ┌──────────┐   │
        │   观察      │──否──│任务完成？│───┘
        │  Observe    │      └────┬─────┘
        └─────────────┘           │是
                                  ▼
                           ┌──────────┐
                           │ 返回结果 │
                           └──────────┘
    """)


# ==================== 第五部分：简单 Agent 实现 ====================


def simple_agent_demo():
    """简单 Agent 实现演示"""
    print("\n" + "=" * 60)
    print("第五部分：简单 Agent 实现")
    print("=" * 60)

    class SimpleAgent:
        """最简单的 Agent 实现"""

        def __init__(self):
            self.tools = {
                "calculator": self.calculator,
                "search": self.search,
            }
            self.memory = []

        def calculator(self, expression: str) -> str:
            """计算器工具"""
            try:
                result = eval(expression, {"__builtins__": {}})
                return f"计算结果: {result}"
            except Exception as e:
                return f"计算错误: {e}"

        def search(self, query: str) -> str:
            """模拟搜索工具"""
            # 模拟搜索结果
            mock_results = {
                "天气": "今天北京晴朗，气温 25°C",
                "新闻": "今日科技新闻：AI 技术持续发展",
            }
            for key, value in mock_results.items():
                if key in query:
                    return value
            return f"搜索 '{query}' 的结果：暂无相关信息"

        def think(self, user_input: str) -> dict:
            """思考：决定下一步行动"""
            # 简单规则判断（实际应用中由 LLM 决定）
            if any(op in user_input for op in ["+", "-", "*", "/", "计算"]):
                return {
                    "action": "tool",
                    "tool": "calculator",
                    "input": user_input.replace("计算", "").strip(),
                }
            elif "搜索" in user_input or "查询" in user_input or "天气" in user_input:
                return {"action": "tool", "tool": "search", "input": user_input}
            else:
                return {"action": "respond", "content": f"我理解你说的是：{user_input}"}

        def run(self, user_input: str) -> str:
            """运行 Agent"""
            print(f"📥 用户输入: {user_input}")

            # 添加到记忆
            self.memory.append({"role": "user", "content": user_input})

            # 思考
            thought = self.think(user_input)
            print(f"🤔 思考结果: {thought}")

            # 执行
            if thought["action"] == "tool":
                tool_name = thought["tool"]
                tool_input = thought["input"]
                print(f"🔧 调用工具: {tool_name}({tool_input})")

                result = self.tools[tool_name](tool_input)
                print(f"📤 工具返回: {result}")

                response = result
            else:
                response = thought["content"]

            # 保存响应
            self.memory.append({"role": "assistant", "content": response})

            return response

    # 演示
    agent = SimpleAgent()

    print("\n📌 测试 1：计算任务")
    print("-" * 40)
    result1 = agent.run("计算 15 * 8 + 20")
    print(f"✅ 最终回复: {result1}")

    print("\n📌 测试 2：搜索任务")
    print("-" * 40)
    result2 = agent.run("查询今天的天气")
    print(f"✅ 最终回复: {result2}")

    print("\n📌 测试 3：普通对话")
    print("-" * 40)
    result3 = agent.run("你好，我想学习 Agent")
    print(f"✅ 最终回复: {result3}")


# ==================== 第六部分：练习与思考 ====================


def exercises():
    """练习题"""
    print("\n" + "=" * 60)
    print("练习与思考")
    print("=" * 60)

    print("""
    练习 1：扩展工具
        为上面的 SimpleAgent 添加一个新工具，比如：
        - 翻译工具（模拟）
        - 日期时间工具

        ✅ 参考答案：
        ```python
        from datetime import datetime

        class EnhancedAgent(SimpleAgent):
            def __init__(self, name: str):
                super().__init__(name)
                # 注册新工具
                self.register_tool("translate", self.translate)
                self.register_tool("datetime", self.get_datetime)

            def translate(self, text: str, target_lang: str = "en"):
                '''模拟翻译工具'''
                translations = {
                    "你好": "Hello",
                    "世界": "World",
                }
                return translations.get(text, f"[{target_lang}] {text}")

            def get_datetime(self, format: str = "%Y-%m-%d %H:%M:%S"):
                '''获取当前日期时间'''
                return datetime.now().strftime(format)
        ```
    
    练习 2：改进决策逻辑
        使用关键词匹配之外的方式来决定使用哪个工具。

        ✅ 参考答案：
        ```python
        from langchain_google_genai import ChatGoogleGenerativeAI

        class SmartAgent(SimpleAgent):
            def __init__(self, name: str):
                super().__init__(name)
                self.llm = ChatGoogleGenerativeAI(model="gemini-1.5-flash")

            def think(self, query: str) -> str:
                '''使用 LLM 决定工具'''
                tools_desc = "\\n".join([
                    f"- {name}: {func.__doc__}"
                    for name, func in self.tools.items()
                ])
                
                prompt = f'''
                可用工具：
                {tools_desc}
                
                用户请求：{query}
                
                返回最合适的工具名称（只返回工具名）：
                '''
                return self.llm.invoke(prompt).content.strip()
        ```
    
    练习 3：集成真实 LLM
        将 think() 方法改为调用 OpenAI API 来决策。

        ✅ 参考答案：
        ```python
        from langchain_google_genai import ChatGoogleGenerativeAI

        def think_with_llm(self, query: str) -> tuple:
            '''使用 LLM 进行推理和决策'''
            llm = ChatGoogleGenerativeAI(model="gemini-1.5-flash")
            
            prompt = f'''
            你是一个智能助手，可以使用以下工具：
            - calculator: 数学计算
            - search: 搜索信息
            - translate: 翻译文本
            
            用户请求：{query}
            
            请分析请求，返回格式：
            工具：[工具名]
            参数：[参数]
            '''
            
            response = llm.invoke(prompt).content
            # 解析响应提取工具和参数
            return parse_tool_call(response)
        ```
    
    思考题：
    ────────
    1. Agent 的记忆系统应该如何设计才能高效？
       答：短期记忆保存最近对话，长期记忆做向量化存储和检索，
       工作记忆跟踪当前任务状态。

    2. 如果工具执行失败，Agent 应该如何处理？
       答：可以重试、尝试其他工具、或向用户报告错误并请求澄清。

    3. Agent 最多应该执行多少步？为什么需要限制？
       答：通常限制 5-10 步，防止无限循环和过度消耗资源。
    """)


# ==================== 主函数 ====================


def main():
    """主函数"""
    print("🤖 Agent 基础概念")
    print("=" * 60)

    what_is_agent()
    agent_vs_llm()
    agent_components()
    agent_loop()
    simple_agent_demo()
    exercises()

    print("\n" + "=" * 60)
    print("✅ 课程完成！下一步：02-react-agent.py")
    print("=" * 60)


if __name__ == "__main__":
    main()
