"""
DPO 训练
========

学习目标：
    1. 理解 DPO 原理
    2. 掌握偏好数据准备
    3. 使用 TRL 进行 DPO

核心概念：
    - Direct Preference Optimization
    - 偏好对 (chosen/rejected)
    - RLHF 替代方案

环境要求：
    - pip install trl transformers peft
"""

import os
from dotenv import load_dotenv

load_dotenv()


def dpo_overview():
    """DPO 概述"""
    print("=" * 60)
    print("第一部分：DPO 概述")
    print("=" * 60)

    print("""
    Direct Preference Optimization (DPO)
    ────────────────────────────────────
    
    无需训练奖励模型，直接优化策略模型以符合人类偏好。
    
    
    RLHF vs DPO
    ───────────
    
    ┌─────────────────────────────────────────────────────────┐
    │                    RLHF (复杂)                          │
    │                                                         │
    │   偏好数据 ─▶ 训练奖励模型 ─▶ PPO训练 ─▶ 对齐模型        │
    │                    │              │                     │
    │                    ▼              ▼                     │
    │              需要维护两个模型，训练不稳定                 │
    └─────────────────────────────────────────────────────────┘
    
    ┌─────────────────────────────────────────────────────────┐
    │                    DPO (简单)                           │
    │                                                         │
    │   偏好数据 ─────────────────▶ DPO训练 ─▶ 对齐模型        │
    │                                 │                       │
    │                                 ▼                       │
    │                    直接优化，训练稳定                    │
    └─────────────────────────────────────────────────────────┘
    
    
    DPO 原理
    ───────
    
    核心思想：将奖励模型的优化转换为策略模型的优化
    
    目标：最大化 chosen 回答的概率，最小化 rejected 回答的概率
    
    Loss = -log(σ(β × (log π(chosen) - log π(rejected) 
                       - log π_ref(chosen) + log π_ref(rejected))))
    
    其中:
    - π: 当前策略模型
    - π_ref: 参考模型 (冻结的 SFT 模型)
    - β: 温度参数，控制偏离程度
    """)


def preference_data():
    """偏好数据"""
    print("\n" + "=" * 60)
    print("第二部分：偏好数据准备")
    print("=" * 60)

    print("""
    数据格式
    ───────
    
    {
        "prompt": "请解释什么是机器学习",
        "chosen": "机器学习是人工智能的一个分支，它使计算机能够...",
        "rejected": "机器学习就是让机器自己学习"
    }
    
    
    数据来源
    ───────
    
    1. 人工标注
       - 给定 prompt，人工比较两个回答
       - 质量最高，成本最高
    
    2. GPT-4 生成
       - 使用 GPT-4 生成 chosen 回答
       - 使用较弱模型生成 rejected 回答
    
    3. 模型生成 + 人工筛选
       - 模型生成多个回答
       - 人工选择最好和最差的
    
    
    数据质量原则
    ───────────
    
    1. Chosen 和 Rejected 要有明显区分
    2. 对比应该基于关键质量维度:
       - 准确性
       - 有用性
       - 安全性
    3. 避免过于相似的回答对
    """)


def dpo_with_trl():
    """使用 TRL 进行 DPO"""
    print("\n" + "=" * 60)
    print("第三部分：使用 TRL 进行 DPO")
    print("=" * 60)

    code_example = """
    from trl import DPOTrainer, DPOConfig
    from transformers import AutoModelForCausalLM, AutoTokenizer
    from datasets import load_dataset
    from peft import LoraConfig

    # 1. 加载 SFT 模型作为起点
    model_name = "./sft_model"  # 或 SFT 后的模型
    model = AutoModelForCausalLM.from_pretrained(model_name)
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    tokenizer.pad_token = tokenizer.eos_token

    # 2. 加载参考模型 (冻结)
    ref_model = AutoModelForCausalLM.from_pretrained(model_name)

    # 3. 加载偏好数据集
    dataset = load_dataset("Anthropic/hh-rlhf", split="train")

    # 数据格式应包含: prompt, chosen, rejected

    # 4. LoRA 配置
    peft_config = LoraConfig(
        r=8,
        lora_alpha=16,
        lora_dropout=0.05,
        target_modules=["q_proj", "v_proj"],
    )

    # 5. DPO 配置
    training_args = DPOConfig(
        output_dir="./dpo_output",
        num_train_epochs=1,
        per_device_train_batch_size=4,
        gradient_accumulation_steps=4,
        learning_rate=5e-5,
        beta=0.1,              # DPO 温度参数
        max_length=512,
        max_prompt_length=256,
        fp16=True,
        logging_steps=10,
    )

    # 6. 创建 DPOTrainer
    trainer = DPOTrainer(
        model=model,
        ref_model=ref_model,
        args=training_args,
        train_dataset=dataset,
        tokenizer=tokenizer,
        peft_config=peft_config,
    )

    # 7. 训练
    trainer.train()

    # 8. 保存
    trainer.save_model("./dpo_model")
    """

    print(code_example)


def dpo_variants():
    """DPO 变体"""
    print("\n" + "=" * 60)
    print("第四部分：DPO 变体和技巧")
    print("=" * 60)

    print("""
    DPO 变体
    ───────
    
    1. IPO (Identity Preference Optimization)
       - 简化 DPO 的数学推导
       - 更稳定的训练
    
    2. KTO (Kahneman-Tversky Optimization)
       - 不需要成对数据
       - 只需要标记好/坏
    
    3. ORPO (Odds Ratio Preference Optimization)
       - 不需要参考模型
       - 显存更省
    
    
    超参数调优
    ─────────
    
    1. β (温度参数)
       - 较大 (0.5): 更保守，接近参考模型
       - 较小 (0.1): 更激进，偏离更大
       - 推荐: 0.1 - 0.5
    
    2. 学习率
       - 通常比 SFT 低: 1e-5 到 1e-4
    
    3. 训练轮数
       - 通常 1-3 轮
       - 过多容易过拟合
    
    
    最佳实践
    ───────
    
    1. 确保从良好的 SFT 模型开始
    2. 使用高质量偏好数据
    3. 监控 chosen/rejected 的概率分离度
    4. 注意防止过拟合
    """)


def exercises():
    """练习题"""
    print("\n" + "=" * 60)
    print("练习与思考")
    print("=" * 60)

    print("""
    练习 1：构建偏好数据
        为特定任务构建 100 条偏好对

        ✅ 参考答案：
        ```python
        import json
        
        def build_preference_data(prompts: list, llm_good, llm_bad):
            preference_data = []
            
            for prompt in prompts:
                # 使用强模型生成 chosen
                chosen = llm_good.invoke(prompt).content
                
                # 使用弱模型生成 rejected
                rejected = llm_bad.invoke(prompt).content
                
                preference_data.append({
                    "prompt": prompt,
                    "chosen": chosen,
                    "rejected": rejected,
                })
            
            # 保存数据
            with open("preference_data.jsonl", "w") as f:
                for item in preference_data:
                    f.write(json.dumps(item, ensure_ascii=False) + "\\n")
            
            return preference_data
        
        # 或使用人工标注方式
        def collect_human_preference(prompt, response_a, response_b, human_choice):
            if human_choice == "A":
                return {"prompt": prompt, "chosen": response_a, "rejected": response_b}
            else:
                return {"prompt": prompt, "chosen": response_b, "rejected": response_a}
        ```
    
    练习 2：DPO 训练
        在 SFT 模型基础上进行 DPO

        ✅ 参考答案：
        ```python
        from trl import DPOTrainer, DPOConfig
        from transformers import AutoModelForCausalLM, AutoTokenizer
        from datasets import load_dataset
        
        # 加载 SFT 模型
        model = AutoModelForCausalLM.from_pretrained("./sft_model")
        ref_model = AutoModelForCausalLM.from_pretrained("./sft_model")
        tokenizer = AutoTokenizer.from_pretrained("./sft_model")
        
        # 加载偏好数据
        dataset = load_dataset("json", data_files="preference_data.jsonl")
        
        # DPO 配置
        config = DPOConfig(
            output_dir="./dpo_model",
            beta=0.1,
            learning_rate=5e-5,
            num_train_epochs=1,
            max_length=512,
        )
        
        # 训练
        trainer = DPOTrainer(
            model=model,
            ref_model=ref_model,
            args=config,
            train_dataset=dataset["train"],
            tokenizer=tokenizer,
        )
        trainer.train()
        ```
    
    练习 3：评估对比
        比较 SFT 和 DPO 模型的效果

        ✅ 参考答案：
        ```python
        def compare_models(sft_model, dpo_model, test_prompts: list, evaluator_llm):
            results = {"sft_wins": 0, "dpo_wins": 0, "tie": 0}
            
            for prompt in test_prompts:
                sft_response = sft_model.generate(prompt)
                dpo_response = dpo_model.generate(prompt)
                
                # 使用 GPT-4 评估
                eval_prompt = f'''比较以下两个回答，哪个更好？
问题: {prompt}
回答A: {sft_response}
回答B: {dpo_response}
输出: A/B/Tie'''
                
                result = evaluator_llm.invoke(eval_prompt).content
                if "A" in result:
                    results["sft_wins"] += 1
                elif "B" in result:
                    results["dpo_wins"] += 1
                else:
                    results["tie"] += 1
            
            return results
        
        # 典型结果: DPO 在安全性、有用性方面通常提升明显
        ```
    
    思考题：
    ────────
    1. DPO 和 RLHF 各有什么优缺点？

       ✅ 答：
       DPO 优点:
       - 实现简单，无需奖励模型
       - 训练稳定，超参数少
       - 显存需求较低
       
       DPO 缺点:
       - 对偏好数据质量敏感
       - 可能不如 RLHF 灵活
       
       RLHF 优点:
       - 理论上效果更好
       - 可以在线学习
       
       RLHF 缺点:
       - 实现复杂
       - 训练不稳定

    2. 什么情况下 DPO 效果不好？

       ✅ 答：
       - 偏好数据质量差 (chosen/rejected 区分不明显)
       - SFT 模型本身效果差
       - 偏好标准不一致
       - 数据量过少 (< 1000 条)
       - 过拟合偏好数据
    """)


def main():
    print("🎯 DPO 训练")
    print("=" * 60)
    dpo_overview()
    preference_data()
    dpo_with_trl()
    dpo_variants()
    exercises()
    print("\n✅ 课程完成！下一步：09-model-merging.py")


if __name__ == "__main__":
    main()
