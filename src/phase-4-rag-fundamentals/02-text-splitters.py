"""
文本分割策略
============

学习目标：
    1. 理解文本分割的重要性
    2. 掌握常用分割策略
    3. 学会选择合适的分割参数

核心概念：
    - Chunk：分割后的文本块
    - Chunk Size：块大小
    - Chunk Overlap：块重叠

前置知识：
    - 01-document-loaders.py

环境要求：
    - pip install langchain langchain-community tiktoken
"""

import os
from dotenv import load_dotenv

load_dotenv()


# ==================== 第一部分：为什么要分割 ====================


def why_split():
    """为什么要分割文本"""
    print("=" * 60)
    print("第一部分：为什么要分割文本")
    print("=" * 60)

    print("""
    分割文本的原因：
    ───────────────
    
    1. LLM 上下文长度限制
       - GPT-3.5: ~16K tokens
       - GPT-4: ~128K tokens
       - 长文档需要分块处理
    
    2. 检索精度
       - 小块更容易精确匹配
       - 大块可能包含不相关内容
    
    3. 成本控制
       - 只检索相关的小块
       - 减少 token 消耗
    
    分割策略考虑因素：
    ─────────────────
    
    ┌─────────────────────────────────────────────┐
    │  Chunk Size（块大小）                        │
    │  - 太小：上下文不完整                        │
    │  - 太大：检索不精确，成本高                   │
    │  - 推荐：500-1000 字符                       │
    ├─────────────────────────────────────────────┤
    │  Chunk Overlap（重叠）                       │
    │  - 防止边界处信息丢失                        │
    │  - 通常设为 chunk_size 的 10-20%            │
    └─────────────────────────────────────────────┘
    """)


# ==================== 第二部分：字符分割器 ====================


def character_splitter():
    """字符分割器"""
    print("\n" + "=" * 60)
    print("第二部分：字符分割器")
    print("=" * 60)

    from langchain_text_splitters import CharacterTextSplitter

    text = """人工智能正在改变世界。从医疗诊断到自动驾驶，AI 技术的应用越来越广泛。

深度学习是人工智能的重要分支。它模拟人脑神经网络，能够从大量数据中学习模式。

自然语言处理让机器理解人类语言。GPT、BERT 等模型在文本理解方面取得了巨大突破。"""

    splitter = CharacterTextSplitter(
        separator="\n\n",
        chunk_size=100,
        chunk_overlap=20,
        length_function=len,
    )

    chunks = splitter.split_text(text)

    print(f"📌 分割结果（chunk_size=100, overlap=20）：")
    for i, chunk in enumerate(chunks):
        print(f"\n块 {i + 1} ({len(chunk)} 字符):")
        print(f"  {chunk[:50]}...")


# ==================== 第三部分：递归字符分割器 ====================


def recursive_splitter():
    """递归字符分割器"""
    print("\n" + "=" * 60)
    print("第三部分：递归字符分割器（推荐）")
    print("=" * 60)

    from langchain_text_splitters import RecursiveCharacterTextSplitter

    text = """# 人工智能概述

## 定义
人工智能是研究如何使计算机模拟人类智能的技术。

## 应用领域
1. 自然语言处理
2. 计算机视觉
3. 机器人技术

## 发展历程
人工智能经历了多次发展浪潮，从专家系统到深度学习。"""

    splitter = RecursiveCharacterTextSplitter(
        chunk_size=100,
        chunk_overlap=20,
        separators=["\n## ", "\n", " ", ""],  # 按优先级尝试
    )

    chunks = splitter.split_text(text)

    print("📌 递归分割保持语义完整性：")
    for i, chunk in enumerate(chunks):
        print(f"\n块 {i + 1}:")
        print(f"  {chunk}")


# ==================== 第四部分：Token 分割器 ====================


def token_splitter():
    """Token 分割器"""
    print("\n" + "=" * 60)
    print("第四部分：Token 分割器")
    print("=" * 60)

    try:
        from langchain_text_splitters import TokenTextSplitter

        text = (
            "人工智能和机器学习正在快速发展，深度学习模型如 GPT 和 BERT 已经广泛应用。"
        )

        splitter = TokenTextSplitter(chunk_size=20, chunk_overlap=5)

        chunks = splitter.split_text(text)

        print("📌 按 Token 数量分割：")
        for i, chunk in enumerate(chunks):
            print(f"  块 {i + 1}: {chunk}")

    except Exception as e:
        print(f"⚠️ 需要安装 tiktoken: pip install tiktoken")
        print(f"   错误: {e}")


# ==================== 第五部分：特定格式分割器 ====================


def specialized_splitters():
    """特定格式分割器"""
    print("\n" + "=" * 60)
    print("第五部分：特定格式分割器")
    print("=" * 60)

    print("""
    LangChain 提供的专用分割器：
    
    | 分割器                    | 用途                |
    |--------------------------|---------------------|
    | MarkdownTextSplitter     | Markdown 文档       |
    | PythonCodeTextSplitter   | Python 代码         |
    | RecursiveJsonSplitter    | JSON 数据           |
    | HTMLSectionSplitter      | HTML 页面           |
    | LatexTextSplitter        | LaTeX 文档          |
    """)

    # Markdown 分割示例
    from langchain_text_splitters import MarkdownTextSplitter

    md_text = """# 标题

## 第一节
这是第一节的内容。

## 第二节
这是第二节的内容。"""

    splitter = MarkdownTextSplitter(chunk_size=100, chunk_overlap=0)
    chunks = splitter.split_text(md_text)

    print("📌 Markdown 分割结果：")
    for i, chunk in enumerate(chunks):
        print(f"  块 {i + 1}: {chunk[:40]}...")


# ==================== 第六部分：最佳实践 ====================


def best_practices():
    """最佳实践"""
    print("\n" + "=" * 60)
    print("第六部分：最佳实践")
    print("=" * 60)

    print("""
    选择分割策略的建议：
    ───────────────────
    
    1. 通用文本：RecursiveCharacterTextSplitter
       - chunk_size: 500-1000
       - overlap: 50-100
    
    2. 代码文件：相应语言的 CodeSplitter
       - 保持函数/类完整性
    
    3. 结构化文档（Markdown/HTML）：对应分割器
       - 按章节/标题分割
    
    4. 需要精确控制 Token：TokenTextSplitter
       - 避免超出模型限制
    
    调优技巧：
    ─────────
    
    - 测试不同 chunk_size，评估检索效果
    - overlap 不要太大，避免冗余
    - 保留足够的上下文
    """)


# ==================== 第七部分：练习与思考 ====================


def exercises():
    """练习题"""
    print("\n" + "=" * 60)
    print("练习与思考")
    print("=" * 60)

    print("""
    练习 1：对比分割效果
        用不同 chunk_size 分割同一文档，对比结果。

        ✅ 参考答案：
        ```python
        from langchain_text_splitters import RecursiveCharacterTextSplitter

        text = "你的长文本内容..."

        for chunk_size in [100, 300, 500, 1000]:
            splitter = RecursiveCharacterTextSplitter(
                chunk_size=chunk_size,
                chunk_overlap=int(chunk_size * 0.1)
            )
            chunks = splitter.split_text(text)
            print(f"chunk_size={chunk_size}: {len(chunks)} 块")
            print(f"  平均长度: {sum(len(c) for c in chunks) / len(chunks):.0f}")
        ```

    练习 2：代码分割
        用 PythonCodeTextSplitter 分割一个 Python 文件。

        ✅ 参考答案：
        ```python
        from langchain_text_splitters import PythonCodeTextSplitter

        with open("example.py", "r") as f:
            code = f.read()

        splitter = PythonCodeTextSplitter(
            chunk_size=1000,
            chunk_overlap=100
        )
        chunks = splitter.split_text(code)

        for i, chunk in enumerate(chunks):
            print(f"块 {i+1}:")
            print(chunk[:100] + "..." if len(chunk) > 100 else chunk)
        ```

    练习 3：Token 计算
        计算分割后各块的 token 数量。

        ✅ 参考答案：
        ```python
        import tiktoken
        from langchain_text_splitters import RecursiveCharacterTextSplitter

        # 获取编码器
        encoding = tiktoken.get_encoding("cl100k_base")

        def count_tokens(text):
            return len(encoding.encode(text))

        splitter = RecursiveCharacterTextSplitter(chunk_size=500)
        chunks = splitter.split_text(long_text)

        for i, chunk in enumerate(chunks):
            tokens = count_tokens(chunk)
            print(f"块 {i+1}: {len(chunk)} 字符, {tokens} tokens")
        ```

    思考题：
        1. chunk_size 如何影响 RAG 效果？
           
           ✅ 答案：
           | chunk_size | 优点 | 缺点 |
           |------------|------|------|
           | 小 (100-300) | 检索精确 | 上下文不完整 |
           | 中 (500-1000) | 平衡 | 通用推荐 |
           | 大 (1500+) | 上下文完整 | 检索不精确，成本高 |

        2. 什么情况下需要更大的 overlap？
           
           ✅ 答案：
           - 句子跨块边界时
           - 技术文档有跨段落引用
           - 问答对分布在多行
           - 代码中函数调用跨块时
           - 一般设为 chunk_size 的 10-20%
    """)


# ==================== 主函数 ====================


def main():
    """主函数"""
    print("🚀 文本分割策略")
    print("=" * 60)

    try:
        why_split()
        character_splitter()
        recursive_splitter()
        token_splitter()
        specialized_splitters()
        best_practices()
        exercises()
    except Exception as e:
        print(f"\n❌ 发生错误: {e}")
        return

    print("\n" + "=" * 60)
    print("✅ 课程完成！下一步：03-document-transformers.py")
    print("=" * 60)


if __name__ == "__main__":
    main()
